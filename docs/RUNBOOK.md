...
| `TFT014` — *slot N pid P, byte free* | A participant record nothing will reassign: the process is gone and the kernel has released its lock byte, but its arena record still says `LIVE` (or `RESERVED`) | **Nothing reclaims it on this build.** Count how many the finding says are spent (`N of 64`) — at 64 every further attach fails `NoParticipantSlots`. The only recovery is stopping every attached process so the segment is freed; `SIGTERM` is not one, because nothing installs a handler and the default disposition skips every destructor. `tf_tree participants` shows the same slots as `stale`. See [`0028`](./decisions/0028-the-slot-a-killed-participant-keeps.md) |
