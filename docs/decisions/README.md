...
A significant architectural change starts as a decision record in this folder, not as a PR. Skeleton: [`template.md`](./template.md).

Cite [`docs/PROJECT.md`](../PROJECT.md) (D1-D22 in §5) and [`docs/PHASE1.md`](../PHASE1.md); [`docs/API.md`](../API.md) is what a public-surface decision is checked against; [`docs/PHASE2.md`](../PHASE2.md) §1 holds amendments A1-A8. A `path.rs:NNN` citation in a frozen record is unreliable: read the symbol the prose names.
...
| [`0004`](./0004-builder-time-edge-declaration.md) | builder-time edge declaration; arena sized from declared edges. |
| [`0005`](./0005-the-shared-memory-seam.md) | the `tf_tree` to `tf_tree_ipc` seam: fd passing, claims as leases, reaping, fork poisoning. |
...
| [`0011`](./0011-the-bridge-clock-guard-and-the-static-conflict-disposition.md) | bridge static-conflict disposition and `Strict` startup window; clock half superseded by `0012`. |
| [`0012`](./0012-the-authoritative-clock-jump-signal-and-the-degradation-ladder.md) | authoritative `rcl` clock-jump callbacks first, inference demoted to common-mode rejection. |
| [`0013`](./0013-the-benchmark-gate-never-interpolated.md) | benchmark gate queries off-grid stamps so interpolation runs. |
...
| [`0016`](./0016-portable-simd-and-the-dependency-budget.md) | portable SIMD and the dependency budget; withdrawn. |
| [`0017`](./0017-owned-handles-and-the-lifetime-rule.md) | `Tree::claim_owned` and `OwnedWriter`; no user-storable type carries a lifetime. |
| [`0018`](./0018-blocking-waits-belong-in-the-shim.md) | no blocking primitive in the arena; the timeout lives in the shim. |
| [`0019`](./0019-one-binary-and-topology-you-can-wait-for.md) | no `tf_treed` binary; `tf_tree serve` instead; read-only attach implies `CreatePolicy::Never`. |
...
| [`0028`](./0028-the-slot-a-killed-participant-keeps.md) | the slot a killed participant keeps: reclaim by kernel-lock liveness, participant reaping. |
| [`0029`](./0029-the-topology-lock-is-a-kernel-lock.md) | the topology lock is a kernel lock; `Tree::reparent` decides holder liveness by `F_OFD_GETLK`. |
...
| [`0032`](./0032-the-region-table-was-not-part-of-the-purchase.md) | the region table was not part of the Phase 5 header purchase; the region break is owed (`PROJECT.md` §5.1). |
| [`0033`](./0033-the-identity-record-cannot-name-a-namespace.md) | the identity record cannot name a namespace. |
...
| [`0040`](./0040-the-error-that-cannot-be-returned.md) | hand-written `Display` and `core::error::Error` on the five `tf_tree_core` error enums. |
| [`0041`](./0041-python-declares-a-topology-the-way-everything-else-does.md) | Python `build`/`open` accept topology-config text wherever a pair list was accepted. |
...
| [`0047`](./0047-the-recording-this-reader-would-refuse.md) | the recording crate `PHASE2.md` §10 promised is declined. |
...
| [`0054`](./0054-the-conflicts-the-bridge-counts-and-the-arena-cannot-see.md) | `TFT002`/`TFT003` detected nothing; the bridge's counts reach the catalogue. |
...
- **ready**: every open question the *Decision* depends on is resolved and the implementation plan is concrete.
- **implemented**: code shipped, PRs linked, document frozen. A record is not edited to match the code; a new record supersedes it. The only edits are the status line and an amendment banner signed and dated by a later record.
