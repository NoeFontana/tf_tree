...
| [`0013`](./0013-the-benchmark-gate-never-interpolated.md) | **Draft** — the Phase 1 gate queried on-grid stamps, so `I::eval` never ran and the "150 ns" depth-3 number is really **290 ns**. Two open questions. `API.md` §3.4 depends on its re-baseline: `NS_PER_STEP_ESTIMATE` is re-derived in the same commit. |
