...
| [`0013`](./0013-the-benchmark-gate-never-interpolated.md) | **Draft** — the Phase 1 gate queried on-grid stamps, so `I::eval` never ran and the "150 ns" depth-3 number is really **192.7 ns** (median of nine pinned runs; the record's own draft figure of 290 was a `cargo bench --quick` artifact and is superseded by its *Re-baseline* section). The stamp fix and the measurement have landed, and `NS_PER_STEP_ESTIMATE` was re-derived 55 → 64 in that commit as `API.md` §3.4 requires. **What is still open is the gate**: `PHASE1.md` §11.3's thresholds are untouched and the two threshold questions block `ready`; two further questions are recorded, including which *call shape* the budget is written against — the same fold measures 147.6 ns inlined and 200.3 ns behind an `#[inline(never)]` call. Note the process irregularity stated in its header: plan items 1 and 2 were implemented while the record was `draft`. |
