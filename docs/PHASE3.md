...
- A scaling test: 1/2/4/8 threads calling `plan.at` on a shared `Tree`, asserting near-linear aggregate throughput on `3.14t` — this is the claim the phase exists to make. **This one does not exist, and the test that looks like it is a different claim.** `tests/python/test_freethreading.py` runs eight threads over one plan and asserts *correctness* (`test_threads_share_one_plan_without_corruption`); it measures no throughput and would pass on a build that serialised every call. `just py-test-freethreaded` runs it on `3.14t`, so the interpreter arm of this bullet is met. **The scaling arm is met too, since 2026-09-11** — `just py-thread-scaling`, registered in `docs/benchmarks/EVIDENCE.md` — but it is a *reporting* recipe and no workflow runs it, so this bullet's "Required CI" is still unsatisfied by one step: there is no job for the free-threaded interpreter beyond `just py-test-freethreaded`. It is the same gap as §12.2 criterion 4 — see that criterion for what this host can and cannot say about it, and why the missing piece is a measurement rather than a machine. **The gap is named in four places, not two**: §11.3's *"§7.3 in full"* clause and §12.1's measurements-table row *"thread scaling 1→8, GIL build and `3.14t`"* both state it as though it happens, and are deliberately left as they are — they are a requirement and a table of what to measure, and annotating each would put four copies of this paragraph in one document.
...
4. **Thread scaling ≥ 6× from 1 to 8 threads on `3.14t`**, and ≥ 6× on the GIL build for batches above the release threshold. **MEASURED 2026-09-11, and the free-threaded half PASSES on the development host.** `just py-thread-scaling`; the harness is `crates/tf_tree_bench/python/thread_scaling.py` and `docs/benchmarks/EVIDENCE.md`'s probe table is the register. On `python3.14t` 3.14.2 with the GIL off, `--release`, batch 64: **1→8 reads 6.17–6.24×, above the 6× floor in every run.** The GIL half at the same batch — an order of magnitude above §6.1's release threshold at depth 3, so it is the half this criterion asks for — reads **5.79–5.96×**, short by under 4% and reported `INVALID` with its margin rather than `FAIL`. 1→4 reads 3.92–3.96× free-threaded, linear to the four *physical* cores.

    **The pass is on four physical cores for eight threads, and the verdict is one-sided, which is what makes that conservative rather than lucky.** Every way this host can be unfair to a scaling *floor* — half the cores the criterion's "8 threads" implies, SMT siblings, another tenant — pushes the reading **down**, so at-or-above is a claim the host can only have understated. That is `docs/PHASE5.md` §9.3's one-sided-budget argument mirrored: a budget admits a pass from above, a floor from below. A miss where the host has a core per thread is a genuine `FAIL` and the harness reports one.

    **Two revisions of this paragraph were wrong in opposite directions and both are recorded rather than quietly replaced.** The first said 6× "cannot" be reached here — over-claimed, since [`PHASE1.md`](./PHASE1.md) §11.3 records this same 4-core host at **5.73×** on the analogous Rust 1→8 row. The second said the threshold was "the part the host blocks" and that a reading here is *"neither a pass nor a fail"*; that was written from a sweep run against a **debug** extension, which reads 0.52 M samples/s against release's 3.33 M and moves the curve from 4.4–5.1× to 6.17–6.24×. A debug build is not a slower release build but a different program — the sentence `Fitness::probe` carries — and the host was carrying the blame for a build profile. **`≥ 8 physical cores` is still the machine that would settle the GIL half**; it is no longer what this criterion is waiting on.
