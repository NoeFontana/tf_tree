...
4. **Thread scaling ≥ 6× from 1 to 8 threads on `3.14t`**, and ≥ 6× on the GIL build for batches above the release threshold. **UNMEASURED — and that is not the same as blocked, which is what it had been recorded as.** The interpreter is here: `python3.14t` 3.14.2 on the development host reports `sysconfig.get_config_var("Py_GIL_DISABLED") == 1` and `sys._is_gil_enabled() == False`, so the free-threaded build this criterion is about is available and nothing has ever run the sweep against it. The **threshold** is the part the host blocks, and it blocks it in **both** directions. 8 threads over **4 physical cores** can exceed 4× only through SMT, so a reading here is *"neither a pass nor a fail"* — which is [`PHASE1.md`](./PHASE1.md) §11.3's ratified wording for the identical situation on the identical host, and the phrasing to copy. **An earlier revision of this sentence said 6× "cannot" be reached here and that over-claimed:** PHASE1 records this same 4-core host measuring **5.35–5.62×** on its criterion benches and **5.73× / 5.20×** pinned on the analogous Rust 1→8 row, so 6× is *near*, not impossible, and telling the next reader it is unreachable would license them to stop measuring. So the verdict is `INVALID` in both directions — a ≥ 6× reading here would not be a pass either — per `docs/PHASE5.md` §9.3's rule that a host condition must not read as the code failing.
