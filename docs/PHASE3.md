...
- A scaling test: 1/2/4/8 threads calling `plan.at` on a shared `Tree`, asserting near-linear aggregate throughput on `3.14t` — this is the claim the phase exists to make. **This one does not exist, and the test that looks like it is a different claim.** `tests/python/test_freethreading.py` runs eight threads over one plan and asserts *correctness* (`test_threads_share_one_plan_without_corruption`); it measures no throughput and would pass on a build that serialised every call. `just py-test-freethreaded` runs it on `3.14t`, so the interpreter arm of this bullet is met. **The scaling arm is met too, since 2026-09-11** — `just py-thread-scaling`, registered in `docs/benchmarks/EVIDENCE.md` — but it is a *reporting* recipe and no workflow runs it, so this bullet's "Required CI" is still unsatisfied by one step: there is no job for the free-threaded interpreter beyond `just py-test-freethreaded`. **Since 2026-09-11 a sweep exists** — `just py-thread-scaling`, registered in [`docs/benchmarks/EVIDENCE.md`](./benchmarks/EVIDENCE.md) — so what is missing is narrower than "the test does not exist": the *measurement* is taken and **no workflow runs it**, so this bullet's "Required CI" is unsatisfied by the CI step and not by the instrument. §12.2 criterion 4 is where the readings and their limits are; this bullet is about whether CI takes them. The gap is also named in §11.3's *"§7.3 in full"* clause and §12.1's measurements table, which are left as they are — a requirement and a list of what to measure are the right places for an unconditional statement, and annotating each would put four copies of this paragraph in one document.
...
4. **Thread scaling ≥ 6× from 1 to 8 threads on `3.14t`**, and ≥ 6× on the GIL build for batches above the release threshold. **MEASURED 2026-09-11 by `just py-thread-scaling`, and this host cannot settle it either way.** The harness is `crates/tf_tree_bench/python/thread_scaling.py`; [`docs/benchmarks/EVIDENCE.md`](./benchmarks/EVIDENCE.md)'s probe row holds the readings and is deliberately their only copy. The free-threaded half **straddles the 6× floor** — most runs above it, not all — and the GIL half (`just py-thread-scaling-gil`, which the sweep gained on 2026-09-11 so that half stops being run by hand against whatever build `.venv` happened to carry) sits consistently below it. **The GIL half's gap is now part-attributed rather than open**: `--call at_into`, which allocates nothing per call where `Plan::at` builds its (N,4,4) result while the GIL is held, reads higher at 1→8 in six of six interleaved pairs and still misses the floor — so roughly half of it is that allocation and the rest is unattributed. `--gate --call at_into` is refused, because this criterion names `plan.at`.

    **A clearing run is a conservative pass, and that is the strongest thing available here.** Every way this host can be unfair to a scaling *floor* — four physical cores for the eight threads the criterion names, SMT siblings, another tenant — pushes the reading **down**, so at-or-above is a claim the host can only have understated. That is `docs/PHASE5.md` §9.3's one-sided-budget argument mirrored: a budget admits a pass from above, a floor from below. What it does **not** license is calling the criterion met: the margin is inside the instrument's own spread, so the answer is *not settled*, and **≥ 8 physical cores is what would settle it**. A miss where the host has a core per thread is a genuine `FAIL` and the harness reports one.

    **The GIL half's shortfall is unresolved rather than the host's**, and saying otherwise was the third wrong version of this paragraph. Its 1→4 arm is as high as the free-threaded one or higher, so the gap opens only at 8 threads, where SMT siblings contend on GIL-held work — and `Plan::at` allocates its `(N,4,4)` output before `fill` detaches, which is code-side GIL-held work. An `at_into` arm, with no per-call allocation, is owed before the core count is charged.

    **Three revisions of this paragraph were wrong, in three different directions, and all are recorded rather than quietly replaced.** The first said 6× "cannot" be reached here — over-claimed, since [`PHASE1.md`](./PHASE1.md) §11.3 records this same 4-core host at **5.73×** on the analogous Rust 1→8 row. The second said the threshold was "the part the host blocks"; that was measured against a **debug** extension, which reads about six times worse *and moves the curve*, so the host was carrying the blame for a build profile. The third said the half simply **passes**; that reading had its timer started *after* the workers were released, which counts work outside the elapsed window and inflates the ratio with thread count — correcting it moved the median down and put one run of five below the floor.

    **The re-cut has a precedent one document over, and it is this record's twin.** [`0013`](./decisions/0013-the-benchmark-gate-never-interpolated.md)'s *Resolution* faced the same unmeasurable "≥ 6× from 1 to 8 threads" in PHASE1 §11.3's criterion 3 and re-cut it to **≥ 2.5× from 1 to 4 threads** on exactly the physical-core argument, retaining the 8-thread figure as informational with its measurement. Whether criterion 4 takes the same shape is still a decision rather than an edit to this line — but it is a decision with a worked answer beside it, not an open one.
...
- [ ] §12.2 gate met, or a written explanation of which criterion failed and by how much — **three states, and this box's wording admits two.** `docs/PHASE4.md` §9's equivalent box already carries this correction: beside **met** and **failed by a stated margin** there is **not producible on this host**, which is a fact about the host and not about the code. Criterion 4 is the worked case as of 2026-09-11 (`just py-thread-scaling`, `just py-thread-scaling-gil`): its free-threaded half *straddles* the floor and its GIL half sits below — partly on a GIL-held allocation the `at_into` arm priced, and partly unattributed — so **neither is "met" and neither is a failure** — it is the third state, with the readings and their limits in [`docs/benchmarks/EVIDENCE.md`](./benchmarks/EVIDENCE.md) rather than restated here, which is how a fourth copy of them went stale in the third digit. The box stays unticked because six of the seven criteria are still unmeasured, and criterion 4 is unsettled rather than resolved
