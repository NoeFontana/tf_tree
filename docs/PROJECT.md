...
- Writing `unsafe` anywhere that is not one of the four boundaries in [`0007`](./decisions/0007-the-unsafe-budget-and-the-c-abi.md) — the arena's memory, the OS, a foreign runtime, a foreign caller — or a second time inside `tf_tree`, whose `#![deny(unsafe_code)]` carries exactly one `#[allow]`: `OwnedWriter`'s lifetime extension, granted by [`0017`](./decisions/0017-owned-handles-and-the-lifetime-rule.md). **It is not yet the only lifetime extension in the workspace, and that is deliberate**: `0017` steps 6–7 delete `tf_tree_c::publisher::extend_to_static` (`crates/tf_tree_c/src/publisher.rs`, called from `publisher.rs` and `bridge.rs`) and `tf_tree_py`'s copy (`crates/tf_tree_py/src/tree.rs`, called from the same file), and they are deferred to their own commits. Until they land the answer to "is there a second lifetime extension here?" is **yes, two, both in bindings** — and no third one may be added
