...
**Phase 2 — shared memory.** `MappedArena` via sealed `memfd`, FD passing over a Unix socket that doubles as the liveness signal, a participant registry with PID-reuse-proof identity, cooperative crash-consistent reaping, and read-only attach as an MMU-enforced safety boundary. Highest technical risk in the project. It was also to ship `tf_tree_record` (the bit-identical replay harness) and a `/tf` ingest bridge so benchmarks run against real robot data; **the recorder is declined by [`0047`](./decisions/0047-the-recording-this-reader-would-refuse.md) — its NORMATIVE bit-identity test is what shipped — and the `/tf` bridge shipped as Phase 4 §5.** Fully specified in `docs/PHASE2.md`, including eight mandatory amendments to Phase 1 that the multi-process crash matrix exposed.

> **Status: the engine half is complete, §3.5 included since 2026-08-28** — amendments A1–A8, rendezvous, attach protocol, liveness, claim leases, reaping, fork poisoning, page population, and CLI adoption, all under [`0005`](./decisions/0005-the-shared-memory-seam.md). **§3.5's ownership migration now ships**, in the shape [`0037`](./decisions/0037-a-takeover-is-not-a-second-open.md) argued for after #275 deleted the unsound one: `Session::take_over_ownership` takes byte 0 on the description the survivor's session already holds, `Tree::inherit_ownership` binds and serves the existing segment, and `Tree::owner_lost` is the trigger that had never existed. **The trigger is caller-driven** — a survivor polls it in its own loop, nothing polls for it, and an arena whose survivors never call it still ends up ownerless; see D16 below. `tf_tree_record` is **declined** by [`0047`](./decisions/0047-the-recording-this-reader-would-refuse.md), not pending — §10's Record and Replay halves are retired and §10(c)'s bit-identity test is met. `/tf` ingest **shipped as Phase 4 §5** (`docs/PHASE4.md` §0.0: *"Done, except §5.9's affinity knobs and §6.3's replay rows"*). The long-running fault harness is `shm_torture`, whose `PHASE2.md` §0.0 row reads *"Done, and since 2026-09-04 it kills the rendezvous owner"* — **and that row instructs a quoter to carry its qualifier**: two of §11.4's four continuous invariants are checked continuously, a third at teardown, and the fourth is not implemented. What remains is `tf_tree serve`, which [`0019`](./decisions/0019-one-binary-and-topology-you-can-wait-for.md) put in place of §9's `tf_treed` and deliberately made optional rather than owed. `docs/PHASE2.md` §0.0 is the authoritative status table.
...
**Pulled forward deliberately:** the `tf_tree doctor` diagnostics land at the end of Phase 1 (they are how Phase 2 gets debugged), and MCAP record/replay ~~lands early in Phase 2~~ **did not land in Phase 2 at all**. Its NORMATIVE half — deterministic replay as the correctness harness for the shared-memory layer — is `crates/tf_tree_cli/tests/replay_bit_identity.rs`; its *reading* half became Phase 5 bag ingestion under [`0006`](./decisions/0006-the-eight-phase-roadmap.md); and the recorder itself is declined by [`0047`](./decisions/0047-the-recording-this-reader-would-refuse.md).
...
One parent per frame. Keeps topology to two dense arrays and the lookup to an array walk. **No uncertainty is stored — not a joint, not a marginal** ([`0009`](./decisions/0009-descoping-phase-6.md)). *Do not* add multi-parent support to "handle" loop closure — that is a factor graph and a different project. *Do not* reintroduce the same use case through copy-on-write branches either, which is how it tried to come back and why `0009` cut them. Document the limitation prominently instead.
...
> `blake3` is a *deliberate* third rather than a drifted-in fourth: §5.1 mandates BLAKE3-256 truncated to 64 bits for frame-name hashing and argues the collision bound, and the hash cannot be a `std` hasher because Phase 2 has two **processes** interning into one arena, so it must be deterministic across them.
...
> **Amended by [`0057`](./decisions/0057-an-owner-is-not-dead-until-its-files-close.md) (2026-09-14) — *exact* and *no timeout to tune* stand; *in microseconds* does not.** Process death closes the socket when the kernel closes the dying process's files, at the end of its exit: after any core dump and after its address space is torn down, and not before a `fork` child that shares the description exits. `0057` measured it from a survivor's end of an owner's connection, on one host: a `SIGKILL`ed 2.7 MiB process was seen in ~0.24 ms median, a 1 GiB one in ~98 ms, and a small `abort()` whose core went to a pipe `core_pattern` in ~1.1 s. A hangup is symmetric, so the owner's view of a participant is the same event — a participant that dumps core keeps its connection and participant byte for its dump, and the owner's hangup-driven reap of it waits that long — but nobody timed the owner's `epoll` path, and what it costs there is reaping latency, not recoverability. **The rule above is unchanged**, and `PHASE2.md` §3.5 now states the survivor's half as NORMATIVE: `owner_lost()` answers `true` once the attach connection has hung up and the last open file description holding byte 0 has closed, and tf_tree adds no delay, heartbeat or timeout to that event. This entry's text is not rewritten, so the sentence above still says *microseconds*; read it through this note.
...
**The queue a request to spend an arena byte joins, opened by [`0032`](./decisions/0032-the-region-table-was-not-part-of-the-purchase.md) part 2.** It lives here rather than inside that record because a list that changes belongs where changes are cheap, and a `ready` record is meant to be stable; `0032` holds the argument for having a ledger, this section holds the entries.

`FORMAT_VERSION = 3` bought the **header** and not the region table (`PHASE5.md` §1.2's retraction), so one more break is owed. The standing instruction is unchanged — **do not add arena fields opportunistically** — but "wait for the next break" now names a queue rather than an indefinite date.
