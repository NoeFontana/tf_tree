...
- Depth-3 hot lookup p50 under **150 ns with `ScLerp`**, under **100 ns with `LerpSlerp`**.
...
A note on the first criterion: `ScLerp` costs roughly 150–200 flops with two transcendental pairs via log/exp, or ~80 flops with one pair via the dual-quaternion route. Depth-3 means three of them. The 150 ns figure already assumes the dual-quaternion implementation; if it comes in slower, that is information about the interpolation cost, not a reason to abandon the design — report it and consider making `LerpSlerp` the default for latency-critical plans.
