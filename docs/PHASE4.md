...
| §5 ROS 2 ingest bridge | **Done, except §5.9's affinity knobs and §6.3's replay rows.** The `rclcpp` half is `ros/tf_tree_ros`, an `ament_cmake` package (not a cargo crate, not under `crates/` — it needs a middleware) reaching the engine only through `find_package(tf_tree CONFIG)`: §5.8 form 3 `BridgeHandle`, forms 2 and 1 from one `rclcpp_components_register_node`, §5.2's QoS on a dedicated callback group, §5.3's GID attribution, §5.4's both-publishers diagnostic, §5.6's startup remap log. Gated by `just ros-build` / `just ros-test` — 4 ctest binaries, 10 tests, every one of them with an applied mutant — and by the two artifact checks `ros/build.sh` makes for forms 1 and 2. §5.8 and §5.9 are **amended**; see their sections and the findings below. The row below records what the Rust half already was. |
