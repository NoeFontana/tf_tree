...
| §5, Rust half (history; the row above is the live status) | **Done, and superseded by the row above — do not read this one as an open item.** §5.8's amendment is implemented and the `rclcpp` half is UNBLOCKED — the topology config format, `TopologyConfig::builder`, the `--discover` collector, `Action::StaticVerified` (`DeclareStatic` reinterpreted as *verify against the declared constant*) and `Action::UndeclaredEdge` + `BridgeStats::dropped_undeclared` are in `tf_tree_bridge`, driven end to end over `testdata/tfstream/indoor_atelier.tfstream` by `tf_tree topology --discover`. What remains of §5 is `rclcpp` alone. The decision half — §5.4 authority, §5.5 clock resets and domains, §5.6 frame names, §5.7 static semantics, §5.9 counters, and the pipeline that orders them — is `tf_tree_bridge`, a **workspace member whose only dependency is `tf_tree` itself** (added by the amendment's work, so a config can produce a `TreeBuilder`), so `just test` and `just lint` cover it on every host. **The C seam that `rclcpp` calls is now written too**: `crates/tf_tree_c/src/bridge.rs`, behind a default-off `bridge` feature, whose `extern "C"` entry points are all in `docs/PHASE4.md` §3.1's *unstable* tier. `tft_bridge_offer` runs names → declared? → kind → static → authority → clock **and the arena write**, and reports a POD outcome whose `const char *` diagnostics are borrowed from the handle; `tft_bridge_attribute` is §5.3's GID → node cache, Rust-side and unit-tested. **§5.6's `tf_prefix` works** — it rewrites the declared topology and the arena as well as the wire — and its remap table crosses the boundary as `tft_bridge_get_remap`, so the *"log the resulting mapping table at startup"* clause is obeyable from C; see below. Gated by `cargo nextest run -p tf_tree_c --features bridge` and `cargo clippy -p tf_tree_c --features bridge --all-targets`, **both now in `just test` and `just lint`** rather than only in `+nightly` recipes, by the Miri row `just c-abi-check` gained, and by `just c-header-check`, which builds with `--features test-hooks,bridge` and compiles the smoke test with `-DTFT_HAVE_BRIDGE` so the generated declarations are exercised from C. What remains of §5 is subscriptions, QoS and the three deployment-form shells — `rclcpp` and nothing else. |
| §6 test plan | Follows its section; §6.3's QoS test is **amended** (as specified it cannot pass) |
| §7 benchmarks and gate | **Partial, and gate criterion 1 is NOT EVALUABLE** — its denominator swings 43% on unrelated edits to the same binary, so neither the 1.020× it once recorded nor the 1.34–1.46× FAIL that replaced it is a statement about the ABI. See *§7 gate criterion 1 cannot be evaluated* below. The ABI's own cost is stable at **+6 ns**. `examples/abi_cost.rs` covers rows 1 and 2. `tft_plan_at`'s ladder is measured; the ratio is not usable. `catch_unwind` costs **+0.6 ns on a real, non-inlinable call**. `examples/bridge_cost.rs` reports the bridge row's per-offer half — one accepted `/tf` transform through every §5 table *and* the arena write, at 20 dynamic edges. It is **not** gate criterion 3: that compares the bridge against a `tf2` consumer and needs a fair comparison machine, which this host is not. Gate criterion 4 (Miri + ASan clean) passes via `just c-abi-check`. Criteria 3 and 6 cannot be honestly produced here — see below. |
...
ladder rather than as the single ratio §7's gate table once named — that table
has since been rewritten to match
[`docs/decisions/0023`](./decisions/0023-the-gate-that-could-not-gate.md), so the
table below is the amendment's measurement rather than a proposal beside an
unchanged gate.
**This paragraph read *"`0023` is the **draft** that would make that wording
change, and until it is `ready` the table below is a measurement, not an
amendment"* until 2026-09-05**, which was wrong twice over by then: the record's
own `**Status:**` line no longer said draft, and the rewrite the paragraph was
waiting for had already landed. **A record's status quoted in
another document is a copy that nothing keeps honest** — read the
`**Status:**` line in the record.
...
`examples/abi_cost.rs` now walks the whole ladder between native Rust and
`tft_plan_at`, one rung per thing the ABI does that native does not, every rung a
subtraction inside one build:

| rung | ns/lookup |
|---|---|
| native Rust (+ mat4 write) | 189.1 |
| + guard built per call | 191.7 |
| + not inlined | 192.2 |
| the ABI, no panic guard | 194.2 |
| `tft_plan_at` | 194.8 |

**Building that ladder moved the baseline.** Before it existed this file had
exactly one call site of `Tree::guard()` and the native arm measured **133 ns**,
giving 1.42–1.46× and a FAIL. Adding a second, wholly independent arm moved the
baseline to **~190 ns** and the ratio to ~1.03×. **The ABI arm did not move:
194–196 ns in every variant measured, the original included.**

So the 43% swing is entirely in the comparand. With one guard call site LLVM can
specialise the hoisted-guard loop — plausibly eliding `Guard::drop` bookkeeping
nothing observes — and with two it cannot. Bisected: it is the second
`Tree::guard()` call site, not the `#[inline(never)]` arm and not which tree is
guarded; a separate, identically-populated tree does not restore the 133 ns.

**Neither reading is a statement about the C ABI, and the gate as written cannot
gate.** A criterion whose denominator moves 43% on unrelated edits to the same
binary is not measuring the thing it names. That is a defect in the gate's
construction. Fixing it means either pinning the comparand against
specialisation, or gating the ABI's *absolute* cost — which is stable to ~1% —
rather than a quotient. Both are §7 changes and belong in a decision record.

**What is stable is the decomposition**, because every rung shares a build: the
ABI costs **about +6 ns**, roughly half of it the per-call `Guard`, the rest
split between handle/layout validation, the un-inlinable call and the panic
guard — each a nanosecond or two. That is a very different picture from "+60 ns,
mostly the guard", and it is the one supported by measurement.

**This does not clear the C++/`memfd` result**, which is a separate
configuration and remains unexplained: `docs/benchmarks/tf2.md` measures a C++
caller at 302 ns against native Rust's 202 on the §11.1 fixture over a shared
arena, and `abi_cost` does not reproduce a gap of that size in-process on a heap
tree. `docs/decisions/0022` carries that, and its premise now needs re-checking
against this finding before anything is designed on it.

### What the old text said, kept because the process failure is the lesson

*The paragraphs below are the retracted text, kept because the half that still
holds is the more important half: **the gate was executed by nothing.** That is
why a stale reading survived, and it is also why the artifact went unnoticed —
a benchmark nobody runs is a benchmark nobody sanity-checks. `just abi-cost`
fixes that part and it stands.*
...
>
> The unit is transforms **by choice, and this paragraph used to say it was by
> necessity.** `0011` justified the ordinal by `tf_tree_bridge` having no clock at
> all, which stopped being true when
> [`0012`](./decisions/0012-the-authoritative-clock-jump-signal-and-the-degradation-ladder.md)
> gave the crate an injected steady clock (`SteadyNanos`); `0011`'s own scoping
> note carries that correction and this copy of the reason did not. What is
> unaffected is the *consequence*: `0011` §*Known limitations* records that an
> ordinal is a poor proxy for a duration here, which is why the explicit close is
> the primary mechanism and the backstop is only a backstop.
...
- [x] `tf_tree.h` frozen and reviewed by hand — not merely `cbindgen` output — with every entry justified — **the hand review is a committed list, which is what makes it reviewable rather than remembered.** `xtask/src/headers.rs` carries `STABLE` / `UNSTABLE` / `TEST_ONLY`, and every exported symbol must be in exactly one: `check_partition` fails in *both* directions, so a newly added `extern "C" fn` fails the check until somebody puts it in a tier. Two more checks exist because the partition held by accident once — `check_overlap` reads the *generated* headers back and fails on a symbol **defined** in both (the signature of a `STABLE` omission, whatever kind of item it is, which `check_partition`'s `extern "C" fn` scan cannot see), and `check_stable_is_complete` catches the same omission with the opposite sign, a `STABLE` entry in neither header. The headers are **committed**, so an ABI change is a diff somebody approves; `cargo xtask headers --check` is what keeps the committed pair honest, and it is the first line of `just c-header-check`, which then compiles *and executes* both headers under gcc and clang as C11 and C++17. Runs in `ci.yml`
- [x] `tf_tree_unstable.h` gated behind `TFT_ENABLE_UNSTABLE` — `crates/tf_tree_c/include/tf_tree_unstable.h` opens with `#ifndef TFT_ENABLE_UNSTABLE` / `#error "tf_tree_unstable.h has no stability guarantee; #define TFT_ENABLE_UNSTABLE to accept that"`, so the gate is a translation failure and not a convention. Compiled both ways by `just c-header-check`, in `ci.yml`
...
- [x] All five layouts byte-pattern tested; `QVEC7_XYZW` documented as the Eigen/Sophus path — **and the box's "five" is stale — the header defines more than that.** `TFT_LAYOUT_QVEC7_WXYZ_TWIST6` arrived with §3.6's minor ABI bump and is not in this sentence; recount from `grep -n 'define TFT_LAYOUT_' crates/tf_tree_c/include/tf_tree.h` rather than from the word. The coverage is split, because the layouts are not one kind of thing: `the_four_f64_pose_layouts_describe_the_same_transform` (`crates/tf_tree_c/tests/live.rs`) cross-checks the four `f64` pose layouts from one call, `layout::tests::affine12_is_f32_row_major_without_the_bottom_row` pins the narrowed `f32` one against a hand-written pattern (it cannot be compared bit for bit against the others), and `the_twist_layout_writes_the_pose_and_the_twist_contiguously` asserts the twist layout's pose half byte-for-byte against `QVEC7_WXYZ`. The `live.rs` test's own doc comment carries that split, and records that its name once said five. `QVEC7_XYZW` is documented in the header as **Eigen/Sophus coefficient order**. **Where the three run is not one answer, and this box gave one until 2026-09-05.** `layout::tests::affine12_is_f32_row_major_without_the_bottom_row` is a `#[cfg(test)]` unit test and does run in `just test`. The two `tests/live.rs` tests do not: that file opens `#![cfg(feature = "test-hooks")]`, `test-hooks` is in no `default` set, and no line of `just test` passes it — `cargo nextest list --workspace` lists neither name, and `cargo nextest list -p tf_tree_c --features test-hooks` lists both. They run under `just c-abi-check`, whose Miri `--test live` row and whose ASan row both carry `tf_tree_c/test-hooks`, in `ci.yml`'s `c-abi` job. **The clause this replaces read "Runs in `just test`"** — which is the defect this sweep exists to correct, committed by the sweep; the ROS boxes below state the same kind of distinction for a package `just test` cannot compile at all
- [ ] `static_assert` guards for Eigen and Sophus with a working fallback — **the fallback half is met and the guard half is met for Eigen only, which is a property of Sophus rather than an omission.** `tf_tree.hpp` carries a real `static_assert(sizeof(Eigen::Isometry3d) == 128, …)`, an `alignof`-divides-`sizeof` assert and a C++17 over-aligned-`new` assert. For Sophus there is **no** `static_assert` on the layout and the header says why in place: `offsetof` on Sophus's private members does not compile, so `detail::sophus_is_directly_writable()` is a **run-time probe, taken once and cached** — it reads the public `so3().data()` / `translation().data()` addresses instead of reaching inside the type. A Sophus layout change is therefore caught at run time rather than at compile time, and *that residual is what is missing here*. The generic write path is still compile-time: `static_assert(raw_writable<T>::value, …)` plus a size assert, so a type mistake does not compile. Both blocks are behind `#ifdef TF_TREE_HAS_EIGEN` / `#ifdef TF_TREE_HAS_SOPHUS`, so the header compiles with neither library present — exercised by `just cpp-check`'s build matrix, which runs in `nightly.yml`
- [ ] `find_package(tf_tree)` works from a clean `colcon` workspace with no manual cargo step — **two arms, neither is the other, and no single run is this box.** It is left unticked for the reason the launch-file box below is: the property is exercised and the artefact the sentence names is not. **It was ticked on 2026-09-05 on the strength of the first arm, and unticked the same day** when the second arm was read rather than assumed. `just cmake-check` (in `ci.yml`) configures and installs `crates/tf_tree_c` into a scratch prefix and then builds a downstream project that reaches the library only through `find_package(tf_tree CONFIG REQUIRED)`, static and shared; the consumer runs no cargo, because `crates/tf_tree_c/CMakeLists.txt` builds the Rust library itself (corrosion when it is found, a custom command shelling to cargo otherwise). The colcon arm is `ros/tf_tree_ros`, whose `CMakeLists.txt` likewise names only `find_package(tf_tree CONFIG REQUIRED)`; `ros/build.sh` provisions the prefix ahead of it and hands the archive over through `TF_TREE_PREBUILT_DIR`, so **what colcon exercises is the prebuilt path, not the source path**: `ros/build.sh` runs `cargo build --release -q -p tf_tree_c --features bridge,shm` itself, which is the manual cargo step the box says there is not, and that provisioning is the box below. **So CI holds the first arm and not the conjunction**: `just cmake-check` proves CMake-builds-cargo without colcon, and `just ros-test` proves colcon-finds-the-package with cargo already run. `just ros-test` is container-only and is `ci.yml`'s `tf2` job; **`just ros-build` is in no workflow** — `ros-test` runs `./ros/build.sh --test`, which is the same build plus its ctests, so nothing is lost, but this clause named both recipes as if CI ran both
- [ ] Prebuilt static libs for `x86_64` and `aarch64` published alongside the source path — **the consumption half is built and the publication half is not.** `crates/tf_tree_c/CMakeLists.txt` declares `TF_TREE_PREBUILT_DIR`, prefers a prebuilt when one is found, names the directory when it is not, and propagates a per-target `TFT_HAVE_SHM` probe; `ros/build.sh` uses it. What no release artifact carries is `libtf_tree_c.a`, the two C headers, `tf_tree.hpp` or the CMake package: `just release-archive` builds `--bin tf_tree` and stages the binary plus the licences and README, and `release.yml` uploads that tarball and its checksum. A ROS shop still needs rustup, which is the need §4.4's bullet exists to remove
- [x] Bridge QoS regression test present and passing — `ros/tf_tree_ros/test/test_qos.cpp`, and §6.3's amendment is why it is more than one test: the specified test (broadcaster publishes and **exits**, bridge starts after) fails against a *correct* bridge, because DDS `TRANSIENT_LOCAL` durability is scoped to the **writer**'s lifetime. What is asserted instead is the alive-and-silent broadcaster, with a `volatile` subscriber in the same run as the negative control — the half that actually catches the regression — plus a negotiation-based test reading every field of both QoS rows back off `Subscription::get_actual_qos()`, because reliability has no delivery consequence a test can wait for. §6.3's amendment states why neither style subsumes the other: the durability mutant dies in both, the reliability and depth mutants only in the second. Container-only: `just ros-test`, in `ci.yml`'s `tf2` job
- [x] Multi-publisher detection verified against a deliberately broken launch file — **both halves, since 2026-09-11.** The in-process half is `ros/tf_tree_ros/test/test_attribution.cpp`'s `a_second_publisher_on_one_edge_is_dropped_and_both_nodes_are_named`. The launch half is `ros/tf_tree_ros/test/test_two_broadcasters.launch.py`, which is simultaneously the deliberately broken launch file and the test that reads it: `tf_tree_bridge` plus **two separate broadcaster processes** (`test/conflicting_broadcaster.cpp`, installed to `lib/tf_tree_ros` under `BUILD_TESTING` only) publishing the same `odom -> base_link` over a real RMW, asserted through §5.4's operator-visible `RCLCPP_ERROR` in the bridge's own log. **This box previously read *"the property is verified; the artefact the box names does not exist"*, and `find . -name '*.launch*'` found nothing anywhere in the repository — that is no longer true.** Measured in `docker/tf2` on 2026-09-11: the bridge logged `/ekf and /odom_node have both been publishing odom -> base_link. /ekf owns it; /odom_node's samples are dropped.`, with real names rather than a placeholder, and `just ros-test` reports 31 tests, 0 failures. **Writing that test found a stale spelling in this document.** §5.4 above, and §13's degrade-gracefully line, say an unresolved publisher is attributed to `"<unknown publisher>"`. That string is a literal **nowhere** in the Rust: `crates/tf_tree_bridge/src/lib.rs:285` renders a publisher with no GID as `<unattributed>`, and one whose GID the graph has not resolved renders as its **full GID key** `<gid:...>` — whose own comment says it is deliberately "not `<unknown publisher>`", because two unnamed publishers must stay distinguishable in a diagnostic whose subject is two publishers. So the prose describes a rendering the design replaced, and it cost this fixture an assertion that could never fire. The prose is left for a pass that can decide whether §13's *requirement* moves or the code does; what is corrected here is this box, which is what a reader arrives at from the test. **Not vacuous, and checked rather than assumed**: removing the second broadcaster from the launch description makes it fail on the 60 s `assertWaitFor` timeout — the anti-vacuity check this repository has caught itself skipping three times.

  Two design points the fixture had to get right, both of them scars already recorded elsewhere in this repository. **The two processes must share a clock**: `test_attribution.cpp`'s own comment records that per-broadcaster stamp counters made the second publisher's samples refused as **non-monotonic** (§5.5) before the authority table saw them, so the fixture reads `node->now()` in both processes instead. And **the second broadcaster waits for discovery before its first sample** (`start_delay_s`), because §5.4's log fires on `first_time` per edge — a conflict caught before the GID resolves is logged as `<unknown publisher>` and **never logged again**, though the internal record is rewritten; §5.4's own paragraph above (`freezes <unknown publisher> in as the owner for the life of the bridge`) is the same observation from the other side. Without the delay the fixture would be measuring discovery latency.

  What this does **not** supply is §1 criterion 3, which asks for a bug found in a *host* system; no fixture supplies that.
- [x] §7 gate met, or a written explanation of which criterion failed and by how much — **the explanation, criterion by criterion. Three states and the box's wording admits two**, so the third is named rather than folded into "failed": **met**, **failed by a stated margin**, and **not producible on this host** — a fact about the host and not about the code. **That citation used to read `docs/PROJECT.md` §6 and there is nothing there**: §6 is *Design smells — stop if you catch yourself doing these*, and no section of `PROJECT.md` discusses host-blocked measurement at all. The vocabulary is real but it lives elsewhere, in two places: `docs/PHASE5.md` §9.3's `Sensitivity` axes and its **one-sided-budget** amendment, which is what admits a budget on an unfit host and states the limits of doing so; and `tf_tree_bench`'s own `Ground` enum, which makes a row's reason a value the tool re-derives rather than a sentence somebody re-reads. It has **seven** variants and they are worth reading rather than paraphrasing: `HostFitness`, `HostCores`, **`Tf2NotLinked`**, **`FrozenBackendNotCompiled`**, `MeasuredElsewhere`, `NoInstrument`, `MeasurementRefused`. The two in bold are separate on purpose and a first draft of this sentence folded them into one "a feature not linked" — they are exactly the pair this repository's stale reasons have already confused, the committed `.tft` rows having blamed the wrong one, which is this box's own subject. Read the amendment before classifying anything as host-blocked: on this host, `fair_for_ratios` and `fair_for_memory` are **true**, so "fails `Fitness::probe`" names one axis of four and not a machine that cannot measure.

  | criterion | verdict | evidence |
  |---|---|---|
  | 1. C ABI cost | **partly met — R1, R2 and the control pass; R3 is reported and not gated** | `just abi-cost` at `[profile.embedder]`. §0.0's subsection above carries R1 at 1.025–1.038× against a **provisional** 1.10, R2 at 0.999–1.006×, and the control at 0.992–1.002×. R3's allowance is deliberately unset: the toy fixture's 1.25 was set against a ~16 ns numerator and does not carry to §11.1's, where the same quantity is 48.2–62.5 ns. The single quotient this criterion used to be is **withdrawn** by [`0023`](./decisions/0023-the-gate-that-could-not-gate.md), and neither reading taken against it — the 1.020× PASS nor the 1.34–1.46× FAIL — is a current statement about the ABI. **`just abi-cost` runs in no workflow**, so this criterion is gated locally; its readings are registered in [`EVIDENCE.md`](./benchmarks/EVIDENCE.md) and `just evidence-audit` is what keeps that register from going stale, which exists because the example was executed by nothing for months while this section printed its number as a PASS |
  | 2. C++ wrapper within 2% of the C ABI | **met, in both error modes** | `just cpp-bench`, which like `just abi-cost` above **runs in no workflow** — `grep -o 'just [a-z0-9-]*' .github/workflows/*.yml` does not name it — so this row too is gated locally. 1.006× with exceptions and 1.006× under `-fno-exceptions`; the shortfall the second used to show was NRVO, and the finding is written up above rather than summarised here |
  | 3. bridge CPU below one `tf2` consumer's | **not producible here** | it compares the bridge against a `tf2` consumer and needs a fair comparison machine, which this development host is not. `examples/bridge_cost.rs` measures the bridge's own per-offer half and is **not** this criterion |
  | 4. zero ASan/UBSan findings across the C and C++ suites | **met** | `just c-abi-check` (Miri + ASan, in `ci.yml`) and `just cpp-check`'s ASan/UBSan build (in `nightly.yml`). There is deliberately no Rust UBSan row — `rustc -Zsanitizer` has no `undefined` — so the UBSan half lives where there is C++ to check |
  | 5. §6.4 differential to 1e-12 | **the tolerance is pinned and the ROS end-to-end path is not** | `tf_tree_bench::differential::TOLERANCE` is `1e-12` and the differential harness runs against `tf2::BufferCore` in the container (`just tf2-check`). What §6.4 asks for beyond that is a *bag replayed through the bridge* compared over the same query set, and §6.3's last two rows — bag replay at 10× and the three deployment forms agreeing byte for byte — are the ones §0.0 records as not done |
  | 6. §1's operational criteria including the surprise log | **open, and not satisfiable by code** | the box below |

- [ ] §1 operational criteria met, including the surprise log committed to the repository — **not satisfiable by code, and this is the box that gates Phase 7.** Criterion 1 is two weeks of continuous operation on real hardware; criterion 3 asks for at least one pre-existing bug found in the host stack by the multi-publisher detection, stated as a falsifiable prediction (if two weeks finds nothing, either the detection is broken or the host stack is unusually clean — determine which). Criterion 2's *mechanism* exists and its *log* does not. §1's amendment says every entry is filed against a [`PHASE7.md`](./PHASE7.md) §4 J-row or opens a new one; `.github/ISSUE_TEMPLATE/surprise.yml` is the intake, and what it does about that rule is carry a `jrow` field — *"Does a row in `PHASE7.md` §4 already cover this?"* — beside its own observation fields, so an entry either names a J-row or declares that it opens one. **This clause read *"its fields are that table's columns in order so an entry transcribes without re-typing"* until 2026-09-05, and that was invented**: the form's fields and §4's table columns are neither the same set nor the same order, and the sentence was written in the same commit that flagged an enumeration elsewhere in this box as incomplete. Read the form; and two observations have already been promoted to records of their own ([`0017`](./decisions/0017-owned-handles-and-the-lifetime-rule.md), [`0018`](./decisions/0018-blocking-waits-belong-in-the-shim.md)) because they turned out to be about our design rather than about `tf2`'s behaviour. What `grep -rIl surprise` over this repository does **not** find is a committed log — every hit is the machinery, the specs that describe it, or a source comment citing it. **Run the grep rather than reading a list here** — the hits move whenever anything cites the log, and an enumeration kept in this file is a second thing to maintain. §1's own rule holds: do not proceed to Phase 5 having only met criterion 1
