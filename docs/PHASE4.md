...
| §4 C++ wrapper, CMake package | **Done.** Header-only `tf_tree.hpp`, both error modes, Eigen and Sophus interop, `find_package(tf_tree CONFIG)` proved by a downstream consumer. §6.2's matrix — {gcc, clang} × {C++17, C++20} × {exceptions, `-fno-exceptions`} — is `just cpp-check`, 8 builds all **run**, plus ASan/UBSan clean. §7 gate 2 **passes at 1.003×** against a 2 % allowance. |
