...
| §4 C++ wrapper, CMake package | **Done, with one gate open.** Header-only `tf_tree.hpp`, both error modes, Eigen and Sophus interop, `find_package(tf_tree CONFIG)` proved by two downstream consumers (static and shared). §6.2's matrix — {gcc, clang} × {C++17, C++20} × {exceptions, `-fno-exceptions`} — is `just cpp-check`: 8 builds all **run**, plus ASan/UBSan clean. **§7 gate 2 passes for the default build (0.99–1.01×) and fails for `-fno-exceptions` (1.03–1.08×)** — see below. |
...
### §7 gate criterion 2 is open for the `-fno-exceptions` build

`plan.at<Eigen::Isometry3d>` against `tft_plan_at`, interleaved within each
round so drift is common-mode:

| build | ratio | gate < 1.02 |
|---|---|---|
| exceptions (default) | 0.989 – 1.005 | **PASS** |
| `-fno-exceptions` | 1.029 – 1.082 | **FAIL** |
...
Three causes were found, fixed, and measured:
...
**What remains is unexplained**, and three further hypotheses were measured and
refuted: the return object's size (a heap-allocated `Error` made it *worse*,
1.038×), the payload move (in-place construction did not move it), and the
`optional` discriminant. The structural difference left is that the mode
returns a 456-byte object where the other returns 128. It is recorded as open
rather than closed on a favourable run.
