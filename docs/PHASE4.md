...
- [ ] Multi-publisher detection verified against a deliberately broken launch file — **the property is verified; the artefact the box names does not exist.** `ros/tf_tree_ros/test/test_attribution.cpp` carries `a_second_publisher_on_one_edge_is_dropped_and_both_nodes_are_named`, a gtest that constructs two publishers on one edge in-process, and it runs under `just ros-test`. `find . -path ./target -prune -o -name '*.launch*' -print` finds nothing anywhere in this repository, `ros/` included — so what is missing is a launch fixture that starts two broadcasters as separate processes over a real RMW, which is a different thing from two publishers in one. **Half of that is not container-blocked and the box does not say so.** A `.launch.py` and the second broadcaster node are ordinary files that can be *written* anywhere; what needs the container is *running* them, and the recipe that would run them (`just ros-test`) already exists and already runs in `ci.yml`'s `tf2` job. So the split is: writing the fixture is doable work on any host, executing it is CI's, and no measurement is owed by anybody — which is a different status from the one "the artefact does not exist" implies. §1 criterion 3 is untouched by either: it asks for a bug found in a *host* system, which no fixture supplies
