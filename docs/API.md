...
| 20 | Python exceptions carry the fields a handler branches on, and five classes are added: `TimeDomainMismatchError`, `EdgeAlreadyClaimedError`, `NonMonotonicStampError`, `ArenaHeldButUnreachableError`, `ArenaAbsentError` | Python | R5, [`PHASE3.md`](./PHASE3.md) §4.4 | **landed** — [`0058`](./decisions/0058-the-fields-a-python-exception-only-printed.md) steps 1–8. Attributes are set on the raised instance's `__dict__` with `args` left `(message,)`, so pickling keeps them; an id is the arena's stored names or `None`, never an integer; `ExtrapolationError.domain` is the query's tag; `EdgeAlreadyClaimedError.owner_slot` is `int \| None`, `None` for the `CLAIMING` sentinel; `ArenaHeldButUnreachableError` carries `holder_slots` and `ownership_held` and no pid. Every new class is a direct `TfTreeError` subclass, so no existing `except` clause stops matching. R5's *"not a field"* is D11's rule for a Rust error and does not reach a binding's exception (`0058` §7). `ClaimRevokedError` waits for a Python-reachable trigger, and `PHASE3.md` §11.1 records the four arms no test reaches |
