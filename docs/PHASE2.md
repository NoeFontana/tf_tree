...
> limit of 0, the shell default on both hosts, does not stop a pipe dump; only 1
> does. The limit is set at recipe level, so the binary gains no `unsafe` and CI
...
> a core dump, and a green crash-points run is evidence about neither the
> 2026-09-12 wedge nor the 2026-09-13 one: the first is consistent with a dumping
> heir and was never shown to be one, and a recurrence of its shape under
> `RLIMIT_CORE=1` would refute that. What it gains: the kill-mid-dump source of
> §0.0's *"`aborted` is a floor"* is gone from the recipes, and so is the
> helper's run from every abort. The dumping configuration stays runnable
> without a new flag — the bare binary, invoked outside a recipe, inherits the
> shell's limit.
