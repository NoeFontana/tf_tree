...
**An arena no runtime directory names sits outside this boundary, and putting it back inside by hand is out of contract.** `TreeBuilder::build_shared` creates a segment whose fd is the capability: no directory, no `.lock`, no `.sock` — and therefore no byte for its own participant record to be judged by. Unserved, that is the design, because nothing else can find the segment and no observer holds a probe. Binding an `OwnerServer` over that fd publishes it into a rendezvous anyway, and every peer that joins by name judges liveness by the byte (§5.1): it reads the creator as **dead** while the creator is publishing, and reclaims the record and the claim behind it. [`0031`](./decisions/0031-the-participant-record-with-no-byte.md) decided that composition **out of contract** on 2026-09-18. It is not refused, and no single call can refuse it — the defect belongs to a pair of calls and neither member can see the other. The supported way to serve a created arena is `tf_tree::Open`'s create arm, which takes its lock byte *before* it builds (§3.4).
...
let tree = tf_tree::Open::new()
    .domain(7)
    .name("robot")?                          // a name is validated, so it is a Result
...
| Holder dies without unlocking | lock released by the kernel, immediately at the end of the holder's exit — after any core dump and the teardown of its address space (timed on 6.8, ordering read from kernel source; [`0057`](./decisions/0057-an-owner-is-not-dead-until-its-files-close.md)) |
...
(the hatch working), `a_live_byte_0_refuses_both_policies`
...
poll(OUR OWN attach socket, timeout 0) for POLLHUP/POLLERR   -- the caller's loop; nothing polls for it
  ...
  hangup    -> F_OFD_GETLK(byte 0)                             -- 0043: is the ROLE vacant?
               held -> somebody already took over, or is mid-bind -- or a fresh
                       open() holds it in passing through §3.4 steps 2-4 and
                       will hand it back (0057). Nothing to do THIS call; the
                       ...
                       otherwise: F_OFD_SETLK(byte 0) ON THE DESCRIPTION THIS
                         ...
                         acquired  -> bind a pid-suffixed socket, listen, rename it
                                      ...
                                      on any failure: release byte 0, restore the
                                                      ...
                                                      participant
                         contended -> another survivor is taking over, or a fresh
                                      open() holds the byte in passing; KEEP OUR
                                      SLOT. The next poll's GETLK sees the byte
                                      held and says "nothing to do" by itself if
                                      an heir took it, or sees it free again if a
                                      passer gave it back, and we try again -- so
                                      no single non-Inherited answer is final.
                                      That is what the earlier "retry connect with
                                      backoff" was for, reached without a
                                      reconnect (0043, 0057).
```
...
**NORMATIVE.** `owner_lost()` answers `true` once the survivor's attach connection has hung up and the last open file description holding byte 0 has closed ([`0043`](./decisions/0043-owner-lost-is-a-question-about-the-owner.md)). For a dying owner that is the end of its exit, which includes any core dump and the teardown of its address space; a `fork` child sharing those descriptions holds them until it exits. tf_tree adds no delay, heartbeat or timeout to that event (D17). *[`0057`](./decisions/0057-an-owner-is-not-dead-until-its-files-close.md) Decision 5; §3.7 step 9 carries one host's figures, and no test bounds the duration, because it is the host's: its `core_pattern`, its page size, and the owner's resident memory.*
...
**Recovery capacity is whatever was attached and eligible at the instant the ownership role fell vacant, and it cannot be added afterwards — NORMATIVE** ([`0055`](./decisions/0055-the-recovery-capacity-a-fleet-cannot-add-later.md) part 1). An ownerless arena with any participant byte still held admits no new **rendezvous** attachment *to itself*: nothing is serving, so §3.7's join cannot start, and §3.4's split-brain check refuses an ordinary create. The rendezvous is the only door a would-be heir can come through — `Tree::attach_shared` and `attach_shared_at` both refuse `AttachMode::ReadWrite` before they map anything (`refuse_a_byteless_writer`, `ShmError::ReadWriteNeedsRendezvous`), and `attach_joined_at` is `pub(crate)` with one caller — so **the set of processes that could inherit *that* arena can only shrink from that instant on**.

**§3.4's `CreatePolicy::Always` hatch still creates in this state, and it is not a counterexample — read it as the opposite.** It skips the split-brain check by design (`self.create != CreatePolicy::Always && lock.any_participant_held()?`), takes byte 0 and the creator's slot, and binds a **fresh** segment over the rendezvous name: it *abandons* the held arena rather than joining it, leaving its survivors publishing into a segment nobody else can reach. Its creator is an `Attachment::Owner`, so `inherit_ownership()` answers `NotApplicable` — the hatch replaces an arena and never becomes its heir. `the_escape_hatch_creates_over_a_stranded_participant` is the test, and `docs/RUNBOOK.md` states the cost. **An earlier revision of this paragraph said the check refuses the create full stop, which that green test refutes and §3.4 and §0.0's `--force-new` row both contradict.** A fleet that intends to survive owner death holds that capacity *before* the owner dies.

**The axis is eligibility, not mode**, and there are three ways to be ineligible. The first is the only one an operator can *see* from outside — a census reports modes, never loops or lifetimes — which is why the other two are stated here and in `docs/RUNBOOK.md` as requirements to provision rather than causes to diagnose:

1. **Read-only.** `inherit_ownership()` answers `Inheritance::ReadOnly` and does nothing else: an owner writes the participant table on every grant and a `PROT_READ` mapping cannot. A fleet of consumers cannot rescue itself, and that is D18 working rather than failing.
2. **Read-write and never polling.** The trigger is the caller's, so an attachment that never evaluates `owner_lost()` is not recovery capacity however it is mapped. This is the one an integrator will not derive from the paragraph above, because "open one read-write" reads like the whole remedy and is only half of it.
3. **Not attached at that instant.** Eligibility is a property of an attachment's *lifetime*, not of a fleet's declaration: a supervisor-restarted publisher is read-write and polling and is still not capacity, because the door closed while it was down.

A fourth cause exists and is deliberately not on this list: a participant built against a release before 2026-08-28 cannot inherit at all, whatever it is or does. That is a release question rather than a fleet one ([`0055`](./decisions/0055-the-recovery-capacity-a-fleet-cannot-add-later.md) part 1), and `docs/RUNBOOK.md` carries it as its own triage bullet — so the runbook lists four where this list has three, on purpose.

**So a census is not the measure — the instant is.** `tf_tree participants` shows who is attached; it cannot show who is looking, and it cannot show who was attached when the byte fell free. Nothing in the library enforces any of this, and that is the same ceiling the paragraph above lives with: `0055` answers its question 1 *"no mechanism"* precisely because every candidate that could enforce it is the thread or daemon this section refuses.
...
**Step 9:** the socket is not a handshake channel to be closed after use — it is how a participant learns the *owner* has died, with no polling. **It learns it once its attach connection has hung up and the last open file description holding byte 0 has closed (§3.5's NORMATIVE sentence), not when the owner is signalled** — this read *"in microseconds"* until 2026-09-13. The kernel closes a dying process's files, releasing byte 0 with them, only after writing any core dump and tearing down its address space. Measured on one host ([`0057`](./decisions/0057-an-owner-is-not-dead-until-its-files-close.md)): a `SIGKILL`ed 2.7 MiB owner was seen in ~0.24 ms median, a 1 GiB one in ~98 ms (dirty 4 KiB pages; far less under `transparent_hugepage=always`), and a 2.7 MiB `abort()` whose core went to the host's pipe `core_pattern` in ~1.1 s (~1.2 s at 1 GiB); for that whole window no survivor could inherit and every fresh join was refused. A `fork` child that shares the owner's descriptions holds the socket and byte 0 until it exits (§6.2 for the lock bytes; [`RUNBOOK.md`](./RUNBOOK.md)'s fork section for the socket). Participant death is detected by the lock file; owner death is detected by the socket. Both are kernel-maintained; neither involves a timeout.
...
> **Erratum (2026-09-18, [`0055`](./decisions/0055-the-recovery-capacity-a-fleet-cannot-add-later.md) step 7): the code meets neither clause of the paragraph above, and one of them it deliberately stopped meeting.** *Both hashes* was never met — `IpcError::HandshakeRejected` carries the owner's `format_version` and `layout_hash` and not the client's, and `tf_tree_ipc` depends on `rustix` and `libc` alone, so it cannot read this build's constants to print beside them; closing that needs two more fields on a published crate's public variant. *"Must say exactly that"* **was** met, by a per-status remedy inside the message, and step 7 moved it to [`RUNBOOK.md`](./RUNBOOK.md)'s `HandshakeRejected` table — because seven such remedies rendered messages of 112 to 378 bytes into a 256-byte C buffer, four of which truncated. That is the shape [`0059`](./decisions/0059-the-arena-errors-that-cannot-describe-themselves.md) settled for the sibling `ShmError::LayoutMismatch`, and `0059` already records this same sentence as stale for it. **The rule this paragraph should state** — facts plus the variant name as the runbook's search key, remedy in the runbook — is `0059`'s convention (g); amending the paragraph to say so is a spec change and is not made here.
...
| `takeover.after_ownership_lock_before_bind` | ownership released; another participant takes over; joiners retry. **`another participant` is a precondition, not an outcome**: the corpse leaves nothing behind — its one shared mutation is `F_OFD_SETLK` on byte 0 of the description its session already held, which the kernel undoes at death, with no socket, no `rename`, no participant-byte change and no arena write — but the role is re-taken only by a process that is *already attached*, because only `is_joined()` attachments can inherit and an ownerless arena admits no new one. With **zero** attached read-write survivors this row's repair does not apply and the state is absorbing; `a_killed_heir_leaves_the_role_for_the_next_survivor` deliberately keeps a second heir attached, and `shm_torture` defers an owner kill rather than taking the last one (2026-09-10) |
...
> **Amendment — the harness pauses its own churn for the width of one reap, and
> §11.4's "attach/detach" clause is what that trades against (2026-09-11).**
>
> `shm-torture-asan` was red on four of five nightlies, classified `POPULATION`
> every time, with the engine exonerated by the trigger tally on each. The cause
> is that the driver's `kill()`-to-`wait()` interval is **exactly** the interval
> in which the owner is dead and *undetectably* dead: `do_exit` runs `exit_mm()`
> before `exit_files()`, so the corpse's socket and participant byte are released
> only after its page tables are torn down, and §3.5's trigger is a socket
> hangup. Through all of it every survivor keeps drawing this workload's
> 2 %-per-operation detach arm, and a survivor that leaves cannot return, because
> §3.4 step 4 refuses `CreatePolicy::Never` against held participant bytes.
>
> **The interval scales with the victim's dirty pages, not with the sanitizer.**
> ~0.09 ms per resident MB: a plain child is 2.8 MB and reaps in 0.3 ms median, an
> ASan child is 43–49 MB and reaps in 6.7 ms. The clause above says "Run it under
> ASan", and that is still right — but the plain build wedges at the ASan job's
> own `--children 4` too (measured, 900 s), so ASan is a window-widener here and
> not the subject.
>
> **What changes in this section's own terms.** A `kill.in_progress` marker is
> written before the signal and removed after the post-reap census, and a child
> checks it immediately before detaching and stays instead. So *"random
> attach/detach"* now means: random attach/detach, except for the width of one
> reap per owner kill. Sub-millisecond normally (§0.0's row measures a plain
> child's reap at 0.3 ms median; this read *"tens of microseconds"* until
> 2026-09-14, [`0057`](./decisions/0057-an-owner-is-not-dead-until-its-files-close.md)), tens of milliseconds under
> ballast, against a detach arm that fires about every fiftieth operation — but it
> is a reduction in what this section asks for, and the run prints how many
> detaches it suppressed so a run leaning on it says so. It suppresses a detach;
> never a kill, an inheritance, or a violation.
>
> **Two positive controls, because this class had none and the obvious one is not
> portable.** `--victim-ballast-mb` gives each child N MB of dirty memory, so the
> window can be widened on a plain release build: 512 wedges an *unfixed*
> `--children 4` run at the third owner kill, in about 25 seconds, and that is how
> this was found. But it depends on the victim's pages being 4 KiB — on a host with
> `transparent_hugepage=always`, which is what GitHub's runners use, 256 MB is 128
> huge pages rather than 65 536 small ones and the reap collapses from ~25 ms to
> 1.2–1.8 ms. `--stop-owner-ms` has no memory physics in it: `SIGSTOP` the owner
> for N ms, and it holds its socket open (so nothing can inherit) while no longer
> serving (so nothing can join). At 300 ms the unfixed harness wedges on its
> **first** owner kill.
>
> Both are siblings of `--inject-violation` and `--no-inherit` — deliberate
> failures, so that a fix can be shown to *stop* something rather than shown not to
> have stopped it yet, which is all a green 30-minute run can say about a defect
> that fires on ~1 % of kills.

> **Amendment — the torture recipes run their children without core dumps
> (2026-09-14, [`0057`](./decisions/0057-an-owner-is-not-dead-until-its-files-close.md)
> Decision 6).**
>
> *"A random crash point armed in 10% of children"* is an `abort()` in each child
> that reaches its site, and `SIGABRT`'s default action is *core*. On a host
> whose `core_pattern` is a pipe — apport on the dev host, systemd-coredump on
> GitHub's runners — the kernel runs the crash helper **before** the child's
> files close, so an armed owner or heir holds its socket and byte 0, and the
> role, for the helper's whole run. A crash-points run's recovery therefore
> depended on the host's helper, which is not what this section measures.
> `just shm-torture`, `just shm-torture-crash-points` and `just
> shm-torture-asan` now run the binary under `prlimit --core=1:1 --`. A soft
> limit of 0, the shell default on both hosts, does not stop a pipe dump,
> measured on both. A limit of 1 does, **measured on kernel 6.8 with apport and
> pending on the runner** (kernel 6.17, systemd-coredump): `0057` step 4 requires
> a `workflow_dispatch` run of its `AN` arm there and the first crash-points
> nightly after landing to read `core_dumped=false` on every reaped armed
> abort. The limit is set at recipe level, so the binary gains no `unsafe` and CI
> inherits it by invoking the recipes; the `[diag] host:` line reads `core_rlimit
> soft=1 hard=1` under them. A `--crash-points` run on a pipe `core_pattern` with
> any other soft limit prints a `[diag] warning:` line and keeps its verdict.
>
> **What this section gives up.** The recipes no longer exercise recovery across
> a core dump, and a green crash-points run is not evidence about the 2026-09-12
> crash-points wedge: it is consistent with a dumping heir and was never shown
> to be one, and a recurrence of its shape under `RLIMIT_CORE=1` would refute
> that. The 2026-09-13 wedge was **not** a crash-points wedge: per job, nightly
> run 34747459144's `shm_torture (30 min)` failed and its crash-points job
> passed, and the plain soak arms no crash points, so no armed abort dumped in
> it. What it gains: the kill-mid-dump source of §0.0's *"`aborted` is a floor"*
> is gone from the recipes, and so is the helper's run from every abort. The
> dumping configuration stays runnable without a new flag — the bare binary,
> invoked outside a recipe, inherits the shell's limit.
...
| `SIGKILL` writer → claim reapable → re-claimed | p50, p99, of a small `SIGKILL`ed victim: a claim lease is released at the same point in the victim's exit as byte 0, so a large or dumping victim adds its teardown or its dump ([`0057`](./decisions/0057-an-owner-is-not-dead-until-its-files-close.md)) | — |
...
| owner kill → new owner serving | p50, p99 | **Done — `just owner-migration`.** Measured from the outside, because that is where the question is meaningful: the driver stamps its `SIGKILL` and then retries `Open::new().create(Never)` until one succeeds, so what is timed is *a fresh process being able to join again* — the thing an ownerless arena refuses. **0.6–1.2 ms p50, 1.1–2.0 ms p99** over five runs of five migrations on this host, each round killing whichever process was serving. **Those are a `SIGKILL` of a small owner, and only that** ([`0057`](./decisions/0057-an-owner-is-not-dead-until-its-files-close.md)): no core dump and a small teardown. Ranges, not a figure: the p50 and p99 above are a scheduler-sensitive measurement on a shared host, and unlike the row below they are a direct timing rather than a quotient, so they are the more quotable of the two. An owner that dumps core or holds a large dirty address space is not inheritable until its exit ends (§3.5's NORMATIVE sentence), and `0057` measured joins refused for the same window: ~1.1 s for a dump through a pipe `core_pattern`, ~98 ms for 1 GiB of 4 KiB pages (quoted in §3.7 step 9) |
...
4. **Kill → re-claimable p99 under 10 ms. MET, and gated in CI since 2026-09-09 — `just gate reclaim-latency must-pass`, in `ci.yml`'s `shm` job on both matrix rows.**

    **It was met and ungated for the life of the project**, which is the part worth recording: `just reclaim-latency` existed, printed a verdict and carried the INVALID/FAIL split that every other gate binary here lacks, and ran in no workflow — so nothing would have gone red if a reap had regressed to a timeout.

    **Why an absolute duration is gateable on a host that fails `Fitness::probe`.** [`PHASE5.md` §9.3](./PHASE5.md#93-honesty-requirements--normative)'s one-sided-budget amendment is the licence. Measured here: 200 trials, 200/200 contended, p50 0.112 ms, p99 0.159 ms, in 0.2 s wall — a ~60x margin against the 10 ms budget, which scheduler noise on a hosted runner cannot flip, while a reap that had fallen back to a timeout would land in milliseconds-to-seconds.

    **What the figure is of** ([`0057`](./decisions/0057-an-owner-is-not-dead-until-its-files-close.md), 2026-09-14). `reclaim_latency`'s victim is a small `SIGKILL`ed child. A claim lease is an OFD lock, released at the same point in the victim's exit as byte 0, so a victim with 1 GiB of dirty 4 KiB pages would pay about 100 ms of teardown first, and one that dumps core its crash helper's run. Neither is the library's, and neither is in this gate: it stays what it is, a measurement of the library's half on a small victim.

    **`may-refuse`, not `must-pass`, and the reason is the point of the contract.** `reclaim_latency` exits `2` for its own non-vacuity refusal — *"N of 200 claims succeeded immediately; this run measured process teardown, not reclaim latency. INVALID, not FAIL."* — which is a statement about the runner and not about the code. `must-pass` maps exit 2 to exit 1, so on a loaded hosted runner a descheduled parent would produce a red job indistinguishable from a genuine p99 regression. **That collapse is the defect `gate.rs` exists to remove, and wiring it as `must-pass` would have reproduced it in the module's own first customer.** The refusal stays visible rather than silently green: `may-refuse` emits a `::warning::` on exit 2. The policy is read by `scripts/gate-run.sh` and not by the workflow, so the job cannot re-spell the contract and drift from it.
...
| `BootIdMismatch` | arena predates a reboot (only possible with a file-backed dev arena) | recreate the arena. **This row is the file-backed case, and `HelloStatus::BootIdMismatch` from the §3.7 handshake is not it** ([`0055`](./decisions/0055-the-recovery-capacity-a-fleet-cannot-add-later.md) step 7): there a *serving owner* answered, which is proof the arena did not outlive a reboot, and the segment would not have survived one. That status means the two processes disagree about which boot this is — one of them could not read `/proc/sys/kernel/random/boot_id`, or the two `boot_id` parsers diverged on it — and [`RUNBOOK.md`](./RUNBOOK.md)'s `HandshakeRejected` row is the triage. *Telling an operator to recreate a live arena is what the deleted per-status advice did.* |
...
- [~] `shm_torture` runs 30 minutes nightly, clean, under ASan — **the ASan arm is not clean, as of 2026-09-10, and the box is `[~]` for that reason.** It ran clean on the nightly of 2026-09-10 05:46 (a `workflow_dispatch`) and failed on the scheduled run three hours later, on the population condition [`0055`](./decisions/0055-the-recovery-capacity-a-fleet-cannot-add-later.md) is about — not on an ASan report and not on an invariant: `Classification: POPULATION`, `Heirs attached at this kill: 0`, §3.5 tally `inherited=9` with zero `err-`. The other two arms (the 30-minute `SIGKILL` soak and `crash-points`) were green on that same run. That record carries the measured margin — one kill in six leaves exactly one heir — and the **discriminator** for deciding whether any future red in this job is that condition or a real one. **What this box waits on changed on 2026-09-18, and the sentence it replaces is quoted rather than deleted.** It read: *“Ticking this box again waits on `0055`, not on a re-run.”* `0055` was then answered: no mechanism, and a fleet that wants to survive owner death holds the capacity itself. **That answer does not tick this box** — it says the red is not a defect in the library, which leaves the harness. A run whose population can reach zero heirs is measuring the fleet `0055` describes rather than the code, so what this box now waits on is whether `shm_torture` guarantees an heir at every kill — at which point a `POPULATION` classification becomes a real failure again — and that is a harness decision nobody has taken. **Three arms now, and the third ran in no workflow at all until 2026-09-04.** §11.4 asks for a random crash point armed in 10% of children as well as the `SIGKILL` soak, and `just shm-torture-crash-points` is that arm: `nightly.yml`'s `crash-points` job runs it, its own job because the build carries `--features shm,crash-points` and the parameters are gentler on purpose. Its exit status now reports what the `§11.3:` line says — see §11.4's amendment. The two ASan/soak arms, unchanged: **two arms, and the second one was two minutes.** `just shm-torture` has defaulted to `--duration 30m` and runs in `nightly.yml`'s `torture` job; `just shm-torture-asan` is in the same workflow's `sanitizers` matrix and defaulted to **120 s**, which is right for a local check and is not what this box asks of a nightly. The matrix now passes `--duration 30m --children 4 --kill-hz 4` explicitly, leaving the recipe's own default short so `just shm-torture-asan` stays two minutes for somebody checking a change. Four children rather than six because ASan's shadow memory and interceptors make six under it a different amount of work than six without, and the 90-minute job budget also has to cover a `-Zbuild-std` rebuild
