...
| Holder dies without unlocking | lock released by the kernel, immediately at the end of the holder's exit — after any core dump and the teardown of its address space ([`0057`](./decisions/0057-an-owner-is-not-dead-until-its-files-close.md)) |
...
| owner kill → new owner serving | p50, p99 | **Done — `just owner-migration`.** Measured from the outside, because that is where the question is meaningful: the driver stamps its `SIGKILL` and then retries `Open::new().create(Never)` until one succeeds, so what is timed is *a fresh process being able to join again* — the thing an ownerless arena refuses. **0.6–1.2 ms p50, 1.1–2.0 ms p99** over five runs of five migrations on this host, each round killing whichever process was serving. **Those are a `SIGKILL` of a small owner, and only that** ([`0057`](./decisions/0057-an-owner-is-not-dead-until-its-files-close.md)): no core dump and a small teardown. An owner that dumps core or holds a large dirty address space is neither inheritable nor joinable until its exit ends — §3.5's NORMATIVE sentence — which §3.7 step 9 measured at ~1.1 s for a dump through a pipe `core_pattern` and ~98 ms for 1 GiB of 4 KiB pages. Ranges, not a figure: this is a scheduler-sensitive measurement on a shared host, and unlike the row below it is a direct timing rather than a quotient, so it is the more quotable of the two |
