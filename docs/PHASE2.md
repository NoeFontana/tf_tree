...
*Every citation in that paragraph was a line number into `tree.rs` until 2026-09-19, and the commit that wrote §3.1's paragraph above moved all of them at once. They are symbols now.*
...
4. **Kill → re-claimable p99 under 10 ms. MET, and gated in CI since 2026-09-09 — `just gate reclaim-latency must-pass`, in `ci.yml`'s `shm` job on both matrix rows.**
    ...
    **Why an absolute duration is gateable on a host that fails `Fitness::probe`.** `PHASE5.md` §9.3's one-sided-budget amendment is the licence: every check the probe fails makes a reclaim *longer*, so a PASS with margin is conservative and a FAIL is not attributable to the runner. Measured here: 200 trials, 200/200 contended, p50 0.112 ms, p99 0.159 ms, in 0.2 s wall — a ~60x margin against the 10 ms budget, which scheduler noise on a hosted runner cannot flip, while a reap that had fallen back to a timeout would land in milliseconds-to-seconds.
