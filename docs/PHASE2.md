...
4. **Kill → re-claimable p99 under 10 ms. MET, and gated in CI since 2026-09-09 — `just gate reclaim-latency must-pass`, in `ci.yml`'s `shm` job on both matrix rows.**
    ...
    **`must-pass`, not `may-refuse`**: a runner that stops being able to evaluate this is a finding rather than a free pass. The policy is read by `scripts/gate-run.sh` and not by the workflow, so the job cannot re-spell the PASS/FAIL/REFUSED contract and drift from it.
