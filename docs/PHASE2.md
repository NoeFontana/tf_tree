...
**§12.3 gate 3 is therefore still partly met, for a reason that has moved.** Zero corrupt reads across a torture run is measured — over ~250 composed `map -> tool` reads per observation round, with a floor that fails the run below 16. What is not met: "every §11.3 crash point recovers", because `shm_torture` still does not arm one (§11.4 asks for a random site in 10% of children, and `crash::SITES` now exists to be read from rather than re-spelled); and "the owner dies mid-run", because the driver still owns the rendezvous. Killing it is now *possible* — a surviving joiner could call `inherit_ownership` — and it is the harness that has not been taught to. |
