...
| Ownership migration (§3.5) | **Not done, and as of 2026-08-27 there is no path at all.** Half of it exists: ownership is byte 0 and the kernel releases it when the owner dies. **The takeover half is deleted** (#275, [`0037`](./decisions/0037-a-takeover-is-not-a-second-open.md)). `tf_tree_ipc::Open` had a builder that took a takeover path — step 3, skipping the split-brain check because a process that already holds the arena cannot create a second one — and it handed back the first *free* participant byte while the caller's arena record was elsewhere (#201). It could not be repaired in place: the declaration it rested on is unverifiable from a new file description, since `F_OFD_GETLK` answers *does anyone else hold this byte*. The builder, the arm and the unit test that reached it are gone, `OpenOutcome::TookOver` has no producer, and §3.4's NORMATIVE pseudo-code carries the amendment. D16 was amended for the old shape and now overstates. **The trigger never existed either**: nothing watches the client socket for `HUP`, so no participant ever *called* that path — `crates/tf_tree/src/open.rs`'s module documentation states this. Observable consequence, measured with `shm_torture` (§11.4): kill the owner and the arena keeps serving lookups exactly as §3.5 promises, but no new process can join it. They win the ownership byte, meet §3.4's split-brain check against the surviving participants' bytes, back off, and time out with `ArenaHeldButUnreachable` — for as long as any survivor lives. |
...
> **Amendment (2026-08-27, [`0037`](./decisions/0037-a-takeover-is-not-a-second-open.md)): step 3 and the takeover branch above are deleted, and §3.5 has no implementation path through this algorithm.**
>
