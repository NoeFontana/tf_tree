...
>
> **And the cost it was avoiding is 4 %.** Measured on this host (release, heap arena, §11.1 fixture, `imu_link <- map`, 256 off-grid stamps, medians of 11 interleaved rounds, pinned): `Tree::lookup` 524–529 ns, `Plan::at` with a hoisted guard 200.8–202.9 ns, `Plan::at` with a guard built in the loop 223.6–225.0 ns. The whole per-call `Guard` is **21.7–22.9 ns, 4.1–4.3 % of `Tree::lookup`**. The contention the withdrawn sentence predicts does not appear at or below this host's physical core count either: on a single-edge plan where every thread credits one shared `EdgeCounters` line, counters cost a flat **~9 ns at 1, 2 and 4 threads**. **This is a hand measurement with no producer**, in the sense `docs/benchmarks/EVIDENCE.md` uses the phrase — `crates/tf_tree_bench/src/bin/counter_cost.rs` gains the convenience-path arm that would make it re-derivable, and until it does, these figures are quoted here and nowhere else. **§5.7's sixteen concurrent readers are not settled by it**: this host has four physical cores, so the flat 1/2/4 result does not extrapolate to sixteen.
>
