...
> work per element than a pose row (measured at ~1.1x), so §6.1's estimate
> under-shoots it; the estimate is deliberately *not* given a layout-dependent
> multiplier, because both sides of that threshold stay far below CPython's
> switch interval and [`API.md`](./API.md) §3.4 is NORMATIVE that
> `NS_PER_STEP_ESTIMATE` is re-derived from `0013`'s re-baseline rather than
> guessed at. And **`tf_tree.build` hard-coded `LerpSlerp`**, so no
> Python-constructed tree could answer a twist at all — `build` and
> `open(create=...)` now take `interp=`, spelled as §4.1's own layout sketch
> spells it, with the default left where it was.
