...
- **Ingest throughput:** §12 criterion 5 had no bullet here either. Time `tf_tree_ingest::run` — *both* passes — against the recording's own stamp span, on a generated corpus at the criterion's own density, in **two** `--max-memory` regimes: one fill pass, and the group count the criterion's own four-hour recording forces. Gate the grouped one. Every premise is checked rather than assumed: each arm asserts the pass count it declares, and the density is measured off the survey and floored, because a sparser corpus reads arbitrarily higher and would pass without checking anything. `crates/tf_tree_bench/tests/ingest_throughput.rs` from `just test` (the binary needs no `shm`); `just gate5` is the gate. [`0050`](./decisions/0050-what-ten-times-real-time-divides.md) is where the four parameter choices are argued.
...
2. `.tft` open time under **10 ms** for a 233 MB index (it is an `mmap` plus header validation; anything more means work is happening that should not). **MET, and gated since 2026-09-05 — `just gate2`.**
   ...
   **The falsifier is `--prefault`, which edits no threshold**: it reads every
   ...
   both gated halves red, neither marginal. Two things **refuse**
   rather than passing: a **gated** run whose page-cache eviction did not take
   (an unwitnessed "cold" number is off by two orders of magnitude and looks
   exactly like a fast open), and a fixture under this criterion's own 233 MB
   scale. The eviction premise is a refusal only under `--gate`; an ungated run
   voids the evicted arm and prints why, because that arm gates nothing and the
   ordinary cause is a **filesystem whose pages are RAM** — `$TMPDIR` is a
   tmpfs on many hosts — which is an environment rather than a defect in this
   code.
