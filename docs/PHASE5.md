...
| §5 Diagnostic counters | **Done**, §5.6 included — see its amendment: the capture is structural, not a step. Structs and regions landed with §1; §5.4's `Guard` accumulation, the error-path increments and §5.5's default-on `counters` feature are wired. **§5.4's NORMATIVE requirement for a *long-lived per-thread* `Guard` is WITHDRAWN (2026-09-09)** — it would have held `lookups_ok` while `note_err` published, inverting `TFT010` and `no_counter_evidence`, and it needs a second lifetime extension. The convenience path keeps its **per-call** `Guard` by decision; §5.4's amendment carries the argument and `just guard-cost` the price. This row read **Done** over that requirement without distinguishing the two halves, which is why the withdrawal is stated here and not only in the prose this row outranks. §5.7's measurement is `cargo run --release -p tf_tree_bench --bin counter_cost`: **no measurable contention at or below the CPU count**, so the sharding fallback is not justified. |
...
- **The `mcap` crate must be taken with `default-features = false`.** Its
  ...
  **Amendment — the consequence is reversed; the rule is not.** This bullet used
  ...
  [`SUPPORT.md`](../SUPPORT.md#msrv-policy) records why the workspace MSRV is 1.87.
  And because `compression` is default-**on**, the
...
> switch interval regardless. And
...
> note, which had scoped an assertion that did not exist. §13's box 4 had recorded this as
...
> **Amendment — this requirement is WITHDRAWN, and the convenience path keeps its per-call `Guard`.**
>
> *The withdrawn sentence read:* **"NORMATIVE:** the convenience path (`tree.lookup(...)`, which currently constructs a `Guard` per call) must hold a **long-lived per-thread `Guard`** alongside its existing per-thread plan cache. Otherwise it flushes on every call and reintroduces exactly the contention this design removes. This also amortizes the epoch validation the `Guard` already performs, so it is a win independent of counters." It is quoted rather than deleted, because a claim silently repaired stops recording that it was ever stronger.
>
> **It contradicts the paragraph above it.** The batching argument is made about a `Guard` that *spans a batch of lookups* and then ends; this asked for one with no end of batch. `Guard`'s `Drop` is the **only** thing that publishes `lookups_ok` (`crates/tf_tree_core/src/plan.rs`, `impl Drop for Guard`), while `note_err` writes the error counters straight through by design. A guard that never ends therefore **holds the denominator and publishes the numerator**.
>
> **What that costs is not hypothetical, and two shipped checks invert.** `TFT010` computes `errs / (errs + lookups_ok)`, so a healthy edge with a handful of extrapolation errors would read **100 %** and fire. `no_counter_evidence` skips every counter check when that sum is zero, so a process hammering the tree would look like one that never ran. Nothing in the catalogue tests would have caught either: they perform no lookups.
>
> **It is also unsound as specified.** `Guard<'a>` carries an `ArenaView<'a>` holding a raw base pointer and references into the `Tree`'s own storage, so a `thread_local!` needs a `Guard<'static>` — a **second lifetime extension**, which `CLAUDE.md` and [`0017`](./decisions/0017-owned-handles-and-the-lifetime-rule.md) make a decision record rather than a patch. `0017`'s soundness argument is an `Arc` that `Tree::lookup` cannot supply: it takes `&self`, and `Tree` is `Send + Sync`, so the handle can be dropped on another thread while a cached guard's destructor still writes into freed memory. Scope-keying does not help — a replaced cache entry is *dropped*, which is the same write.
>
> **And the cost it was avoiding is real but not decisive — measured by the registered artifact, not by hand.** `just guard-cost` (`crates/tf_tree_bench/src/backing.rs`, indexed in [`EVIDENCE.md`](./benchmarks/EVIDENCE.md)) prices exactly this: `Tree::guard()` per call against one hoisted guard. On this host, `release` with counters **on**, it reads **+38.3 ns** on a heap arena (203.1 hoisted → 241.4 per call) and **+29.3 ns** on a memfd one; with counters **off**, +19.1 and +20.0. That matches `backing.rs`'s own summary — a per-call guard is ~17–20 ns and the diagnostic counters roughly double it.
>
> **An earlier revision of this amendment quoted 21.7–22.9 ns and called it 4 % of `Tree::lookup`, and that was wrong in a way worth recording.** It was a hand measurement over `imu_link <- map`, a **three-edge** plan — and `Guard::drop` credits an edge *only when the whole batch went through exactly one*, so on a multi-edge plan the edge-counter flush does not happen at all. It measured the configuration in which the guard does least work and reported it as the cost. The registered artifact uses a single-edge plan, where the flush is real. Where the two disagree, `just guard-cost` is the number, because it is the one somebody can re-run.
>
> **The cost does not decide this anyway**, which is why the correction does not reopen it: building the guard would invert `TFT010` and needs a second lifetime extension. A requirement that breaks a shipped diagnostic is withdrawn whether it costs 4 % or 16 %.
>
> **What this host cannot settle** is §5.7's sixteen concurrent readers: it has four physical cores, so nothing measured here extrapolates to sixteen.
>
> **R2 does not make this a correctness question.** API §1's R2 governs the *hot* tier — `Plan::at`, which takes `&Guard` and builds none — and a per-call `Guard` neither allocates, locks nor converts. `crates/tf_tree/src/cache.rs` already says exactly this.
>
> **What the convenience path guarantees instead**, and what a long-lived guard would have deleted: it credits its denominator on **every call**, visible before any loop ends. Pinned by `the_convenience_path_publishes_its_denominator_on_every_call` (`crates/tf_tree/tests/counters.rs`), whose per-iteration assertion is the whole test — a final total passes against a batched implementation too — and which is mutation-verified against one hoisted guard across the loop, the exact shape this requirement asked for.
...
- **Three-way bit-identity:** replay one recording into `HeapArena`, `MappedArena`, and `FrozenArena`; identical query set; assert bit-identical `f64`. This extends Phase 2 §10 and is the core correctness claim of the frozen format. **Implemented 2026-09-08**, `crates/tf_tree_cli/tests/replay_bit_identity.rs::a_replay_into_heap_mapped_and_frozen_arenas_is_bit_identical`, under `just shm-check`. **It was not implemented before, and what stood in its place did not compose.** The claim was carried by two tests that share neither input: `a_replay_into_heap_and_mapped_arenas_is_bit_identical` gives `heap == mapped` over a *replayed* recording at 5 edge pairs x 300 stamps, and `crates/tf_tree/tests/frozen.rs`'s `a_frozen_lookup_is_bit_identical_to_the_live_one` gives `live == frozen` over a *hand-built* tree at 3 edge pairs x 5 stamps. Both are genuine `to_bits()` comparisons and both are kept — but `H == M` on one recording and `H == F` on a different one, built a different way and queried at different stamps, implies nothing about `M == F`, because there is no common input for transitivity to run through. The new test is the sentence above taken literally: one recording, three backends, one query set. Mutation-verified by freezing before the replay rather than after, which makes the frozen arena answer `None` at every probe and fails the assertion.
...
- **Convenience-path denominator:** assert `tree.lookup` in a loop credits `lookups_ok` **once per call**, visible before the loop ends — the guarantee §5.4's withdrawal keeps and a long-lived per-thread `Guard` would have deleted. *This bullet read* "assert `tree.lookup` in a loop performs O(1) atomic flushes, not O(n)" *until 2026-09-09*, over a requirement now withdrawn — and it asked for a symptom as if it were a property, since nothing counts atomic flushes and the only observable of "O(1)" is a counter that stays stale mid-loop, which is precisely the regression. **Implemented**: `the_convenience_path_publishes_its_denominator_on_every_call` (`crates/tf_tree/tests/counters.rs`), mutation-verified against one hoisted `Plan::at` guard across the loop — the exact shape the withdrawn requirement asked for — which fails on the first iteration.
...
1. **Three-way bit-identity passes. MET since 2026-09-08** — `crates/tf_tree_cli/tests/replay_bit_identity.rs::a_replay_into_heap_mapped_and_frozen_arenas_is_bit_identical`, run by `just shm-check` and by `ci.yml`'s `shm` job.

   **This criterion carried no status line at all until then, while every other criterion in this list carried one** — and the reason it could not have carried an honest one is in §11's bullet: the three-way claim was composed from two tests over two different recordings, two different construction paths and two different query sets, so there was no common input through which `mapped == frozen` followed. The composition is retired; the test is the criterion.
2. `.tft` open time under **10 ms** for a 233 MB index (it is an `mmap` plus header validation; anything more means work is happening that should not). **MET, and gated since 2026-09-05 — `just gate2`.**
   ...
   **Only the resident arm gates, and the reason is in the fault counts.** An
   ...
   See [§9.3](#93-honesty-requirements--normative)'s one-sided-budget amendment.
...
5. Ingest throughput ≥ **10× real time** on a representative recording. **MET, and gated since 2026-09-05 — `just gate5`.** [`0050`](./decisions/0050-what-ten-times-real-time-divides.md) is the record; it answers what the ratio divides, what the density floor is for, why this may be gated on a host that fails the timing probe, and at what pass count the criterion is stated. Read it before changing any of the four.
   ...
   **No interval is published in that table, for criterion 2's reason**: a range
   ...
   The falsifier is a **denser corpus**, which turns the verdict red without editing a threshold (measured 2.6× real time at 40× the declared density); `crates/tf_tree_bench/tests/ingest_throughput.rs` drives it, unfenced, so it runs per-PR in `just test`.
...
- [x] Convenience path keeps its **per-call** `Guard` — **§5.4's NORMATIVE requirement is WITHDRAWN (2026-09-09); read its amendment before reopening this.** The box is closed by the withdrawal plus the test that pins what the withdrawal keeps (`the_convenience_path_publishes_its_denominator_on_every_call`), not by building the guard. The rest of this box is kept as the record of what it used to ask for: *"not implemented, and §5.4 states it as NORMATIVE."* `Tree::lookup_tagged` builds a `Guard` **per call** — `let g = self.guard();` inside its plan-cache closure, `crates/tf_tree/src/tree.rs` — so the flush-on-drop batching §5.4 designs is one flush per lookup on this path. What *is* per-thread is the plan cache (`crates/tf_tree/src/cache.rs`'s `thread_local!`), which is a different structure solving a different problem. [`0022`](./decisions/0022-the-per-call-guard-and-the-unwatched-gate.md) carries a 2026-08-28 correction confirming exactly this and leaves the per-call guard in place deliberately, answering the cost with `tft_plan_at_many` rather than with a longer-lived handle — so **the box and a decision record disagreed about what should exist**. That disagreement is resolved, and in `0022`'s favour: §5.4's requirement is withdrawn, the per-call guard stays, and the measurement is in the amendment. The instruction this clause carried — *"closing this box means amending §5.4 or building the guard, not ticking it"* — is what was followed. §0.0's §5 row reads **Done** over this, because the accumulation half (`Cell<u32>` flushed on `Drop`) *is* wired and the row does not distinguish the two halves
