...
- **The `mcap` crate must be taken with `default-features = false`.** Its
  ...
  **Amendment — the consequence is reversed; the rule is not.** This bullet used
  ...
  evidenced: zstd is checked against a committed fixture compressed by real
  libzstd 1.5.5 (`crates/tf_tree_ingest/testdata/ATTRIBUTION.md`), while lz4 has
  round-trip coverage only, because this host has no `lz4` CLI.
...
> | Candidate | Verdict |
> |---|---|
> | `rusqlite` / `libsqlite3-sys` | Vendors C. `docs/PHASE2.md` §2's no-C-build-step rule already forced `mcap` to `default-features = false`, and §0.0's amendment shows what the rule asks for instead: the zstd and lz4 chunk decoders are pure-Rust crates of ours, so the rule cost nothing in the end. Taking C here would abandon that position for one source of one format. |
