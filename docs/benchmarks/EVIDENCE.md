...
| `tf_tree_bench/src/bin/ingest_throughput.rs` | `just gate5` | PHASE5 §12 gate 5: ingest throughput >= 10x real time on a representative recording. **PASSES with a margin of more than an order of magnitude**, and `docs/decisions/0050` is where the four questions behind that number are answered rather than restated. Development host, `--release`, worst of 3 rounds, on a generated 50-edge x 100 Hz x 32 s zstd corpus (3 370 044 B, page cache warm): the gated **grouped** arm clears the 10x floor by **more than an order of magnitude**, and the in-memory arm is above it too. **No interval is quoted**: a range over a handful of runs is a sample rather than this instrument's spread, and re-running the recipe walks outside one without anything having changed. `just gate5` prints the run's own numbers. **The grouped arm is the gated one**: §12's own four-hour representative recording is 72e6 samples at 64 B and does not fit `DEFAULT_MAX_MEMORY_BYTES`, so pass two takes two groups — a gate measured at one fill pass and the criterion as written are not the same claim. Each arm asserts the pass count it declares and REFUSES otherwise. **The denominator is the recording's own stamp span** (`Survey::span_ns`), and the numerator is `tf_tree_ingest::run` — *both* passes; §12's own 0.048 s figure was a `survey` number. **The density is measured off the survey and floored**, because "10x real time" is a statement about the corpus as much as about the code: a corpus sparser than §12's own 100 Hz x 50 REFUSES. It publishes an absolute duration on a host that fails `Fitness::probe`, under PHASE5 §9.3's one-sided-budget amendment. **The verdict can go red on a corpus rather than on a threshold**: a 40x denser corpus reads 2.6x real time (release) and 0.3x (debug) and exits non-zero. The corpus is *generated*, so nothing is committed and there is no stale fixture; its zstd frames are `ruzstd`'s own encoder's, which makes it a round-trip corpus and not a conformance one. `crates/tf_tree_bench/tests/ingest_throughput.rs` is unfenced and runs per-PR in `just test` — the binary needs no `shm` — and deliberately does **not** assert the green direction: the same measurement reads ~17x in debug, a 1.7x margin a loaded runner could take under the floor. The seeded mutants are listed in its header, and no count of them is written here — this cell said "Three" while the header listed four, having gone stale inside the wave that wrote it |
| `tf_tree_bench/src/bin/frozen_open.rs` | `just gate2` | PHASE5 §12 gate 2: `.tft` open under 10 ms for a 233 MB index. **PASSES**, and the finding is the opposite of the one the criterion hypothesises: `Tree::open_frozen` does no work proportional to the index, so this is a regression guard rather than a discovery. **Two cache states, and the qualification travels with the number.** Development host (AMD EPYC-Milan, 4 physical cores with SMT, ext4 on `/dev/sda1`, `--release`), worst of 8 fresh processes: the **resident** open at 338 MiB is more than two orders of magnitude under the 10 ms budget and a small multiple of the 2.1 MiB fixture's; the **evicted** open is roughly two orders of magnitude slower than resident at 338 MiB and much less so at 2.1 MiB. **No interval is quoted**: a range over a handful of runs is a sample rather than this instrument's spread, and re-running the recipe walks outside one without anything having changed. `just gate2` prints the run's own numbers. **Only the resident arm gates**, on the budget and on a scale-invariance ratio well inside its 4× bound (best of each arm). The evicted arm reports rather than gates: the major-fault count of an open is 1 at both sizes and 0 when resident, so the whole difference is inside the **one major fault**, not in the steps around it: under `strace -T`, `mmap` reads 21 us against 19 us and `madvise(MADV_HUGEPAGE)` 18 us against 16 us across a 2550x range of mapped length, and the 128-byte header `pread64` 193 us against 167 us. How much the kernel reads to satisfy that one fault is a property of the file and the mapping, not of a step `open_frozen` takes. It publishes an absolute duration on a host that fails `Fitness::probe`, under PHASE5 §9.3's **one-sided-budget amendment** — every check that probe fails can only make an open slower, so a PASS with margin is conservative and a FAIL is not attributable to the code; the verdict line prints the fitness reasons either way. **The verdict can go red and each half can do it alone.** `--prefault` reads the index inside the timed region — the regression this gate exists to catch, and it edits no threshold — putting the open tens of milliseconds over the 10 ms budget and an order of magnitude past the 4× scale bound, so both gated halves go red and neither is marginal. A run it cannot evaluate REFUSES rather than passing, and the binary's own header carries the enumeration rather than a count here: a **gated** run whose page-cache eviction did not take (the witness is the child's own major-fault count, and `--no-evict` is the control that fires it; ungated, the evicted arm is voided with a printed reason, because it gates nothing and the ordinary cause is a RAM-backed filesystem rather than a defect), a fixture under the criterion's own 233 MB scale, two fixtures too close in size for the scale-invariance arm to be comparing anything, and a gated run that would PASS against a `--budget-ms` **above** the criterion's own — `SCALE_BOUND` is a constant and the budget was not, so a raised budget was the one argument that could green a failing run. `crates/tf_tree_bench/tests/gate2.rs` drives every one of those through the shipped binary and runs per-PR from `just shm-check`; the mutants that were seeded to confirm it can fail are listed in its own header |
...
| `mlock_probe` | what `mlock` does to an arena-shaped mapping — [`0049`](../decisions/0049-the-flag-that-prefaults-the-arena.md)'s executor, and `docs/API.md` §8.3's, which asserted two syscall behaviours and reproduced none. Six arms plus two organic-pressure arms that need a memory cgroup (`systemd-run --user --scope -p MemoryMax=96M`, and one of them is *expected* to be OOM-killed — the file-backed arm is its positive control). Registered as a probe and run by no recipe on purpose: every answer is a property of the kernel, the cgroup and whether the host has swap, so a recipe asserting one would be a gate about the machine. Linux only | `0049`, API.md §8.3, PHASE2 §7.4 |
...
| `lookup` (bench) | PHASE1 §11.3's depth-3 hot p50 rows, and §11.3's NORMATIVE statement that every latency row this gate bounds is measured with the fold **inlined** names this file as the thing that measures it that way. `docs/PHASE3.md` §"the measurement" quotes its `lookup/depth3/sclerp` row as *the* number, and `0013` re-cut both ceilings against it | xtask bench-gate names it as a manual follow-up; PHASE1 §11.3, PHASE3, `0013` |
| `push` (bench) | PHASE1 §11.2's ns/push table. Its consumer is that table and nothing else: `push_sampler` is a **separate** artifact with a recipe, because an unpaired before/after across two `cargo bench` runs on this host read +47 % where the paired delta reads +1.1 ns, so the two answer different questions and only the paired one has a recipe | (executed by nothing; PHASE1 §11.2 is where its numbers land) |
...
| `soak` under cachegrind | **which line of the lookup path mispredicts**, and whether `bracket`'s mask-select is branchless. It is not: the select compiles to a conditional branch, that one line is 57.2 % of the whole process's mispredicts at a 37.3 % rate on the branch, and the loop back-edge is another 18.1 %. Four spellings of the line were built and measured against each other; the table, the arms and the MSRV question are in [`0053`](../decisions/0053-the-branchless-bracket-that-branches.md). **Not a recipe, because it needs the container and a per-arm rebuild**: `docker/tf2/run.sh` bind-mounts the repo at `/work`, and cachegrind records the *host* path of each source file, so `cg_annotate` finds nothing unless the tree is also mounted at its own absolute path — `docker run --rm -v "$PWD:$PWD" -w "$PWD" -u "$(id -u):$(id -g)" -e ROS_HOME=/tmp/.ros tf_tree/tf2-bench bash -lc 'valgrind --tool=cachegrind --branch-sim=yes --cache-sim=no --cachegrind-out-file=/tmp/cg.out target/profiling/soak --workload robot --duration 3s --interval 2s >/dev/null 2>&1; cg_annotate --show=Ir,Bc,Bcm --sort=Bcm --auto=yes /tmp/cg.out'`. The soak is duration-bounded, so **only per-lookup ratios compare across arms** — the denominator is `sample.rs`'s `if h == 0` branch count and `let half = len / 2` is the iterations-per-lookup control | `0053`, sample.rs's `bracket` doc block, tf2.md ×2, fast-path.md |
...
**Most criterion benches are compiled and executed by nothing, and `push_sampler`
is the exception that shows a bench *can* have a recipe.** *This paragraph opened
"The criterion benches are compiled but never executed by anything", which the
row for `benches/push_sampler.rs` in the table above contradicts in this same
file: `just push-sampler-cost` runs `cargo bench -p tf_tree_bench --bench
push_sampler`.* `grep -n 'cargo bench' justfile .github/workflows/*.yml
xtask/src/*.rs` is the instrument, and no count is copied here.
`cargo xtask bench-gate` runs `cargo bench -p tf_tree_bench --no-run` — it links
the suite, which stops it rotting into non-compiling code, and then does not run
it. A criterion bench still cannot *gate*: `criterion` owns the exit status and
has no verdict to fail on, which is why the two §12 criteria that acquired gates
acquired `[[bin]]`s instead. `read_scaling` in particular backs PHASE1 §11.3's read-scaling criterion,
...
**`lookup` and `push` got their rows on 2026-09-06, and how they went without
one is the sharper half.** Neither was executed by anything and neither was
registered — and `just evidence-audit` was **green over both**, because its
coverage test was plain substring containment: `lookup` was excused by the
recipe names `profile-lookup` and `lookup-tf_tree`, and `push` by
`push-sampler-cost` and by the bare `push:` GitHub Actions trigger key, which
will be in `.github/workflows/*.yml` forever. `xtask bench-gate` names `lookup`
and `read_scaling` two lines apart in the same output, as manual follow-ups to
two UNAVAILABLE gates; `read_scaling` had the row and `lookup` did not. The
script now matches the shapes that *execute* a target rather than its name, and
its header carries the class. **The register side had the same hole and it is
closed in the same change**: a hit had to be a backtick anywhere in this file,
so the paragraph you are reading would itself have re-excused `push` the moment
its row was deleted. A hit is now a table ROW whose first cell names the target,
and that was red-tested by deleting the row and watching the script stay red
with the prose in place.
