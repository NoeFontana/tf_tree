...
| `tf_tree_bench/src/bin/frozen_workers.rs` | `just gate4` | PHASE5 §12 gate 4: 16 workers on one `.tft`, total Pss ≤ 1.2× one worker. **PASSES at 1.024× — with a *Rust* worker**, whose private cost is `p` = 0.36 MiB. The criterion is `S ≥ 74p`, so the verdict is a function of the worker: on the same 338 MiB file, 16 spawned Python workers measure **1.785×** (`p` = 13.44 MiB, needing ~994 MiB of arena). Cite the row with the worker's language attached; §12 gate 4's amendment carries the table, and there is **no recipe for the Python arm** |
