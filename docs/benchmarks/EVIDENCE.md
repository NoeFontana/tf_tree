...
| `tf_tree_bench/src/bin/abi_attached.rs` | `just abi-attached` | Whether the C ABI's cost is the boundary or the language. Answer: the boundary — Rust and C++ agree to 4 ns at `lto = false` |
