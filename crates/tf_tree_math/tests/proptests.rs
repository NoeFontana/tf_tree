...
//!
//! Fixed-seed RNG; `Iso3`/`Quat` are built by `exp_se3` of a random twist.
...
/// A twist `[ω(3), v(3)]`: ‖ω‖ up to ≈ 6.9 (past `π`), translation in `±10`.
...
/// A near-identity twist, forcing the small-angle branches.
...
/// Absolute rotation error plus translation error relative to the operands'
/// scale. Robust to the quaternion double cover.
...
//
// One tight tolerance across the whole domain, including near π
// (`docs/PHASE1.md` §3.3); `sclerp_fast_matches_high_precision_near_pi` pins the
// fast method against a 90-digit oracle.
#[test]
...
// Near-π relative rotations against a 90-digit oracle. Rows are
// `(b_quat, b_translation, s, oracle_quat, oracle_translation)` with a = I.
#[test]
...
//
// Do NOT "fix" `LerpSlerp` so this passes (D5; `docs/PHASE1.md` §3.4).
#[test]
...
/// The rotation matrix of `q`, row-major, written out independently of the code under test.
...
/// `quat_from_rot3` inverts `rot3` over the whole rotation group, taking all four branches.
#[test]
fn prop_quat_from_rot3_inverts_rot3() {
    ...
    runner(2048)
        .run(&twist(), |xi| {
            ...
            // Record the branch so the assertion below is not vacuous.
...
/// Near θ = π, where the trace branch degenerates; the oracle is Rodrigues,
/// sharing no code with the quaternion algebra.
#[test]
