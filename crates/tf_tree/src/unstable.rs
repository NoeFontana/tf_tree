...
//!
//! Specified in `docs/API.md` §2.6; mirrors the C ABI's two-header split
//! (`docs/PHASE4.md` §3.1).
//!
...
//! release**, with no deprecation cycle.
//! release**, with no deprecation cycle and no MSRV or platform promise.
//! Membership test: does its shape follow the arena layout (`docs/PHASE5.md`
//! §1)? [`Plan`], [`Guard`] and [`Stamp`] are stable.
//!
//! ```
...
//!     .build()
// `Tree::arena_view` is gated on this feature too.
//!
...
//! [`Plan`]: crate::Plan
//! [`Guard`]: crate::Guard
/// Raw, read-only access to the arena's tables. **Unstable.**
...
/// Whether an edge is dynamic, static or tombstoned. **Unstable.**
/// [`ArenaView`] read.
