...
//!
//! N processes join one arena through the real rendezvous and hammer it with random
//! claim / push / lookup / release / reap / re-attach while the driver `SIGKILL`s one of
//! them at 1–10 Hz and replaces it. Every reader validates every transform it receives.
//! The output is a verdict, not a benchmark.
//!
//! # What this covers, and what it does not
//!
//! * Random attach/detach/claim/reap/push/lookup and `SIGKILL`, **including the
//!   rendezvous owner** (`kill_the_owner`); under `--crash-points` (needs
//!   `--features crash-points`) a random crash point armed in ~10% of children.
//! * Two of §11.4's four continuous invariants: quaternion/NaN on every read
//!   (`Invariant`), two writers on every push (`work`). Slot leakage is checked once, at
//!   teardown (`check_recovery`); the arena-hash invariant is not implemented
//!   (`PHASE2.md` §0.0).
//!
//! A run without `--crash-points` must not be quoted as §11.3 coverage.
//!
//! # Which of §11.3's thirteen sites this workload can reach
//!
//! Twelve fire (measured with `--crash-site NAME[:nth]`; probe
//! `attach.after_slot_assigned_before_publish` with the bare `NAME`), one does not.
//! "Fires" is not "exercises the row's repair claim"; per-site coverage lives in
//! `tf_tree_core::crash_tests` and `tf_tree/tests/rendezvous.rs`.
//!
//! | Site | Reach |
//! |---|---|
//! | `push.*` x3, `claim.after_cas`, `attach.after_slot_assigned_before_publish`, `reclaim.after_probe_before_cas`, `takeover.after_ownership_lock_before_bind` | live arena; live peers meet the state |
//! | `hangup.after_probe_before_cas` | the owner child, armed like any other; needs `reap_owner` |
//! | `open.after_ownership_lock_before_bind`, `open.after_create_before_bind` | creating owner only; `spawn_owner`'s retry is the next `open()` |
//! | `topo.after_copy_before_publish`, `intern.after_hash_cas_before_id_store` | creating owner only; the named state is not produced |
//! | `topo.holding_lock` | never; nothing reparents the fixed chain |
//!
//! # Killing the owner (§3.5)
//!
//! An **owner child** creates and serves the arena; the driver `SIGKILL`s whichever
//! process holds the role every `--owner-kill-every`. Every child is a potential heir:
//! `work` calls `Tree::owner_lost` and `Tree::inherit_ownership`. Every owner kill must
//! show a fresh process joining again (a real `Open::new().create(Never)` probe) and a
//! survivor recording an inheritance, or the run fails. `--no-inherit` is the negative
//! control (`tests/torture.rs` asserts it). §11.4's ASan clause is
//! `just shm-torture-asan`.
//!
//! # One binary, a `child` mode
//!
//! Children re-exec this binary through `current_exe()`, since recovery is defined by
//! what the kernel releases when a process dies. A child that sees a bad transform prints
//! one `VIOLATION ...` line to stderr and exits `EXIT_VIOLATION`.
...
mod imp {
    ...
    /// A child exits with this after printing a `VIOLATION` line.
    const EXIT_VIOLATION: i32 = 3;

    /// The fewest composed `map -> tool` reads a round must validate for the verdict to mean
    /// anything.
    ...
    /// How long after start-up the first owner kill lands: long enough that the rings are full
    /// when the role first goes vacant.
    const OWNER_KILL_FIRST: Duration = Duration::from_secs(4);

    /// How long a migration is given before the run is failed; matches `owner_migration`'s
    /// deadline.
    const OWNER_RECOVERY_DEADLINE: Duration = Duration::from_secs(10);

    /// Operations one attachment performs before detaching and re-joining, so no single lucky
    /// survivor holds the arena for a whole run.
    const OPS_PER_ATTACHMENT: u32 = 2_000;

    /// How many times the operation cap may be extended for a process serving the rendezvous.
    const MAX_OWNER_CAP_EXTENSIONS: u32 = 10;

    /// The attached read-write population below which the **ordinary** victim draw stops taking
    /// anybody.
    const MIN_ATTACHED_FOR_ORDINARY_KILL: u64 = 3;

    /// How long one scheduled owner kill may stay deferred for want of a second eligible heir
    /// before the run calls it a wedge, in `--owner-kill-every` intervals.
    const OWNER_KILL_DEFERRAL_BUDGET_INTERVALS: u32 = 3;

    /// How soon a deferred owner kill is retried: a deferral waits for a replacement's
    /// handshake (milliseconds locally, ~150 ms on a cold CI runner), so it must not cost a
    /// whole inter-kill period.
    const OWNER_KILL_DEFERRAL_RETRY: Duration = Duration::from_millis(250);

    /// The shortest a deferral budget may be.
    ...
    /// Frames in the torture topology: one chain of four dynamic edges, so a `map -> tool`
    /// lookup composes every edge and a bad sample anywhere shows.
    const CHAIN: &[(&str, &str)] = &[
        ("map", "odom"),
        ("odom", "base"),
        ("base", "arm"),
    ...
    /// The topology every participant agrees on; a participant with a different layout is
    /// refused by the rendezvous.
    fn layout() -> TreeBuilder {
        let mut b = TreeBuilder::new().default_interp(InterpPolicy::LerpSlerp);
        for (parent, child) in CHAIN {
            // 64 slots: the ring wraps constantly, which is where a reader races a
            // writer.
    ...
    /// What a reader checks on every transform (§11.4: no non-unit quaternion, no NaN); it
    ...
    /// Wall-clock nanoseconds, the stamp every participant uses.
    fn now_nanos() -> i64 {
    ...
    struct Args {
        ...
        /// §3.5's negative control: children never call `Tree::owner_lost`, so the run must
        /// fail.
        no_inherit: bool,
        /// `--defer-owner-kills N`: force the first `N` owner-kill attempts to defer.
        defer_owner_kills: usize,
        /// `--crash-site NAME`: arm **this** §11.3 site in every child instead of a random
        /// one in a tenth.
        crash_site: Option<String>,
        /// Megabytes of dirty anonymous memory each child holds: the positive control for
        /// the kill-window class (see [`kill_window_path`]), zero by default.
        victim_ballast_mb: usize,
        /// Milliseconds to hold the owner **stopped** before killing it: the portable
        /// positive control for the same window, zero by default.
        stop_owner_ms: u64,
    }
    ...
    pub(crate) fn main() -> Result<()> {
        ...
        let mut a = Args {
            ...
            inject: false,
            readers_only: false,
            // On by default: an arm that had to be asked for would be absent from the
            // one command §13 names.
        ...
        while let Some(arg) = it.next() {
            ...
            match arg.as_str() {
                ...
                // Self-test: one child publishes a bad transform; the run must
                // FAIL.
                "--inject-violation" => a.inject = true,
                // Negative control for the read floor: children read but never
                // publish, so nothing is validated and the run must FAIL.
                ...
                // [`Args::defer_owner_kills`]); refused without the arm it
                // controls.
                "--defer-owner-kills" => {
                ...
                // cannot arm nothing and read as "unreachable".
                "--crash-site" => {
                ...
                // Recorded, not acted on, so cross-flag validation below runs even
                // for `--help`.
                "-h" | "--help" => help = true,
    ...
    struct Kid {
        proc: Child,
        seed: u64,
        /// Carried per child so a respawn of the injecting slot keeps injecting and the
        /// self-test does not depend on which slot the killer picked.
        inject: bool,
        /// The `TF_TREE_CRASH_AT` this child was started with, so an aborting child is
        /// reported by site name (see [`CrashLedger`]).
        crash_at: Option<String>,
        /// First round at which this armed child showed `CoreDumping: 1`; diagnostic (see
        /// [`note_core_dumping`]).
    ...
    /// What a `--crash-points` run knows about §11.3.
    #[derive(Default)]
    struct CrashLedger {
        armed: usize,
        aborted: usize,
    ...
    /// `armed_site` where the feature exists, `None` where it does not; the `cfg` lives here so
    /// both builds share control flow.
    fn crash_spec(
        enabled: bool,
        forced: Option<&str>,
        rng: &mut Rng,
    ...
    /// (`tf_tree_core::crash::SITES` and `tf_tree::CRASH_SITES`), never re-spelled: a typo
    /// would arm nothing and look clean.
    #[cfg(feature = "crash-points")]
    ...
    /// The runtime directory every process in this run shares, passed in `TF_TREE_RUNTIME_DIR`,
    /// which the rendezvous also keys on.
    ...
    /// Where the current rendezvous holder records its pid, so the driver knows whom to kill
    /// after a migration (a pipe per child would need a protocol that survives `SIGKILL`
    /// mid-line).
    fn owner_pid_path(dir: &Path) -> PathBuf {
        dir.join("owner.pid")
    }

    /// Present exactly while the driver is inside an owner kill: the window in which leaving
    /// the arena is irreversible.
    ...
    /// Where a child records which owner it last attached under (one file per pid, one owner
    /// pid).
    fn attach_dir(dir: &Path) -> PathBuf {
        dir.join("attached-under")
    }

    /// Record the owner this process is about to attach under, **before** the `open()`.
    ...
    /// Where a child records a detach it skipped because a kill window was open, one `<pid>`
    /// line per skip.
    ...
    /// Record that this process inherited the role: one `write_all` of one formatted buffer to
    /// an `O_APPEND` descriptor.
    ...
    /// Where every survivor records what §3.5's trigger answered, one `<pid> <outcome>` line
    /// per evaluation.
    ...
    /// Append one trigger outcome, up to `TRIGGER_LOG_CAP` per process: a survivor evaluates
    /// the trigger at ~1 kHz for as long as the role is vacant, and a truncated count still
    /// separates "nobody asked" from "everybody refused".
    fn record_trigger_outcome(
        ...
        *written += 1;
        ...
        {
            // One `write_all` of one line, like `record_inheritance`, so children's
    ...
    /// [`trigger_tally`] as the one line the teardown summary and the wedge report both print.
    ...
    /// Every process that has ever inherited the role, from the ledger.
    ...
    /// `CLOCK_REALTIME` as `seconds.nanoseconds`: a writer-side stamp is the only order the
    /// driver's stdout and the children's stderr share.
    ...
    /// One diagnostic line from a child as **one** `write(2)`: `eprintln!` writes piecewise and
    /// children share the driver's stderr pipe.
    fn child_diag(body: &str) {
        let line = format!("{DIAG} {body}\n");
        let _ = std::io::stderr().lock().write_all(line.as_bytes());
    }

    /// One diagnostic line from the driver as **one** `write(2)`: `println!` hands the body and
    /// newline to the `LineWriter` separately, so a long line can be split by a child's stderr
    /// line.
    ...
    /// `State/CoreDumping/Threads/wchan` for one process from `/proc`, or `absent`.
    ...
    /// **`docs/decisions/0057` Decision 6's warning: armed crash points on a host whose crash
    /// helper runs inside every armed child's exit.**
    ...
    /// `(soft, hard)` from the `Max core file size` row of `/proc/self/limits`, `?` for a
    /// missing column.
    ...
    /// `[slot:pid, ...]`, `(dead-record)` on a `LIVE` record with a free byte, `(driver)` on
    /// the driver's observer.
    ...
    enum DumpPoll {
        ...
        NotPolled,
        /// Polled once per round and never seen dumping (a dump shorter than a round can be
        /// missed).
        NeverSeen,
    ...
    /// One reap as it happened: the status and two clock reads, nothing that touches a file, so
    /// a site inside [`kill_the_owner`]'s kill window pays only two vDSO reads.
    #[derive(Clone, Copy)]
    struct Reaped {
    ...
    impl Reap {
        /// For a site outside any measured interval.
        ...
        /// For a site inside [`kill_the_owner`]'s kill window: **moves** `crash_at` out so
        /// the capture does not allocate.
        fn taken_from(
            site: &'static str,
            slot: Option<usize>,
            kid: &mut Kid,
    ...
    /// Where a worker that held the rendezvous role records that it **left the role alive**,
    /// one `<pid>` line per departure.
    ...
    /// What the driver knows, at a reap, about the reaped process and the role (see
    /// [`role_left_path`]).
    #[derive(Clone, Copy)]
    enum RoleAtReap {
    ...
    /// Instrument 5's optional half: once per round, for an **armed** child only (a plain soak
    /// does no per-round `/proc` read), records the first round showing `CoreDumping: 1` so
    ...
    /// **Instrument 4: the env var carrying a worker's spawn instant** (wall nanoseconds, set
    /// before `spawn()`), so `fork`, `exec` and start-up are inside the first join's
    /// measurement.
    const SPAWNED_AT_ENV: &str = "SHM_TORTURE_SPAWNED_AT_NS";

    /// Instrument 4's threshold: a healthy rejoin takes milliseconds; a stall that could empty
    /// the pool must outlast a driver round (~80 ms at 6 Hz).
    const SLOW_JOIN: Duration = Duration::from_millis(100);

    /// Instrument 4's per-process line cap.
    const SLOW_JOIN_LINES: u32 = 20;

    /// Instrument 4's **run-wide** line cap; the per-process cap alone bounds nothing (a
    /// 30-minute nightly spawns ~10 800 replacements).
    const SLOW_JOIN_RUN_LINES: u64 = 200;

    /// Instrument 4's run-wide budget, one byte per line claimed, shared with no lock and no
    /// `unsafe`: a child appends one byte through its own `O_APPEND` descriptor and reads the
    /// offset back, which is this line's unique run-wide number.
    ...
    /// Claim this line's run-wide number, or `None` when the budget is spent or the claim could
    /// not be written.
    ...
    /// The driver's closing line for instrument 4, when either cap withheld anything; read
    ...
    struct JoinEpisode {
        ...
        /// `started` to the first attempt (after a role holder's cap exit it also holds
        /// instrument 3's two writes).
        lead: Duration,
        ...
        /// Whether this episode's refusal was printed or withheld, so a refusal loop claims
        /// from the budget once.
    ...
    impl JoinEpisode {
        ...
        /// Both caps, this process's first, so a process that spent its own does not spend
        /// the run's.
    ...
    fn spawn_owner(
        ...
        for attempt in 0..6 {
            ...
            // [diag] Instrument 5 for an owner child that aborted before `ready` (it
            // never held the role).
    ...
    /// The behaviour a child is started with, as one value (`crash_at` is per-child and stays a
    /// separate argument).
    #[derive(Clone, Copy)]
    struct ChildSpec {
    ...
    fn spawn(
        ...
        cmd.arg("child")
            ...
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            // Inherited: a `VIOLATION` line must reach the operator's terminal and the
            // CI log even if the driver is killed before it reports.
            .stderr(Stdio::inherit());
        if spec.inject {
        ...
        // [diag] Instrument 4: stamped last, immediately before `spawn()`, so a first
        // join's `exec_ms` covers the spawn itself.
    ...
    fn drive(a: &Args) -> Result<()> {
        ...
        // The one process that holds the role without inheriting it, so [`inherited_pids`]
        // cannot contain it; the driver knows its pid exactly.
        ...
        // When the current unbroken run of deferrals began, and its attempt count; a clock
        // rather than a count because the retry cadence is not the schedule
        // ([`OWNER_KILL_DEFERRAL_RETRY`]).
        let mut deferring_since: Option<Instant> = None;
        let mut deferrals_in_a_row = 0usize;
        // Every firing of the arm, deferred or not: the §3.5 floor separates "never came
        // due" from "came due and nothing came of it".
        ...
        // A wedge is not a `violations` entry: that bails as an engine crash-consistency
        // failure, while a wedge is a population condition, and conflating them is the
        // INVALID-is-not-FAIL collapse.
        ...
        // The whole run and the last hundred rounds; the second dates the moment an arena
        ...
        // The observer's read total on the round before an owner kill: "the data plane
        // never pauses" is relative to the data plane working.
        while Instant::now() < deadline {
            ...
            // holder and requires what a migration owes.
            if let (Some(every), Some(at)) = (a.owner_kill_every, next_owner_kill) {
                if Instant::now() >= at {
                    ...
                    rounds += m.rounds;
                    println!("{}", m.line());
                    // A deferral re-arms in milliseconds, a kill on the
                    // schedule (see [`OWNER_KILL_DEFERRAL_RETRY`]); clamped to
                    // `every`.
                    ...
                    // A deferral is not a migration and stays out of the
                    // verdict's ledger; it has its own tally.
                    if m.deferred {
                        owner_kills_deferred += 1;
                        deferrals_in_a_row += 1;
                        let since = *deferring_since.get_or_insert_with(Instant::now);
                        // `saturating_mul`: `Duration`'s `Mul` panics on
                        // overflow.
                        let budget = every
                            .saturating_mul(OWNER_KILL_DEFERRAL_BUDGET_INTERVALS)
                            .max(MIN_OWNER_KILL_DEFERRAL_BUDGET);
                        // A deferral has to clear or it is the wedge under
                        // a politer name: it ends the run when nothing is
                        // attached (`heirs_before == 0`) or when it repeats
                        // past the budget.
                        let starved = m.heirs_before == Some(0);
                        // [diag] Instrument 2 at every deferral (the
                        // starved stop included), about one line per
                        // [`OWNER_KILL_DEFERRAL_RETRY`] (~96 for a full
                        // budget): the census *trajectory* is what
                        // diagnoses a wedge.
                        ...
                        // `recovered.is_none()` with `heirs_at_kill == 0`
                        // is absorbing (nothing can join an ownerless
                        // arena).
                        if m.recovered.is_none() && m.heirs_at_kill == Some(0) {
                            // [diag] Instrument 2 at the unrecoverable
                            // stop (post-reap census, `/proc` after the
                            // deadline); one line.
                            driver_diag(&population_diag(
                    ...
                    // A wedge ends the run like a violation, by a different
                    // verdict.
            ...
            // The ordinary draw does not take the role holder: an owner killed here is
            // silent (no census, no `Migration`, nothing requires recovery) and
            // consumes an eligible heir at the worst instant (the population reached
            // zero on the nightlies from 2026-09-07).
            ...
            if pool.slots_alive < MIN_ATTACHED_FOR_ORDINARY_KILL {
                ...
                if Some(kid.proc.id()) == role_holder {
                    ...
                    // had aborted or was still dumping is reaped here as an
                    // ordinary kill (see `reap_diag`).
        ...
        reap_finished(&mut kids, &mut violations, &mut ledger, &dir);
        reap_owner(&mut owner_kid, &mut ledger, &dir);
        // Kill every remaining child *before* the recovery check: "no claim is held by a
        // dead participant" holds only for a quiescent arena.
        let current_owner = read_owner_pid(&dir).or(creating_owner_pid);
        // Whether this run can pin the owner's hangup callback, decided from the ledger
        ...
        // [diag] Instrument 5 at teardown: each wait keeps its status and two clock reads
        // and nothing else; lines are written after `check_recovery`.
        ...
        // A run that migrated cannot pin the collector: the heir is an ordinary worker that
        // sweeps the table several times a second (`reap_participants` is one value in a
        // hundred; `reap_dead` two more) and whose `work` loop ends, leaving no owner.

        // The records no hangup callback can collect, named by pid: every process that has
        // held the rendezvous (the creating owner plus [`inherited_pids`]; a process does
        // not run its own hangup callback).
        ...
        // The owner outlives the workers, as a check rather than tidiness: the role holder
        // runs the hangup callback.
        std::thread::sleep(Duration::from_millis(200));
        ...
        // [diag] What the teardown lines need from each `Kid`; `true` marks the role holder
        // killed last.
        ...
        drop(observer);
        // [diag] Instrument 5's teardown lines and instrument 4's closing line, written
        // once every child is reaped and the ledgers are still on disk.
        {
        ...
        // Read and formatted before the scratch directory is removed: `Scratch::drop`
        // deletes every ledger, so reading at the print site would return an empty tally on
        // every run.
        ...
        if a.crash_points {
            ...
            // Derived from this run, never a literal list, which would go stale when
            // the workload gained an operation.
        ...
        // §3.5 prints per migration, not as a total: a bare "3 owner kills" says nothing
        // about recovery, and a silent zero looks like a disabled arm.
        if a.owner_kill_every.is_some() {
            ...
            // The tally prints whatever the verdict: an empty one is a population with
            // nobody to ask, `contended` or `err-` is the engine refusing.
            println!("shm_torture: §3.5 trigger outcomes: {trigger_outcomes}");
            // Both of these reduce what the run did, so they print even as zeroes: a
            // kill rate that cannot be audited is a gate doing less than it claims.
            println!(
            ...
            // The third reduction (see [`kill_window_path`]), printed for the same
            // reason.
            println!(
        ...
        // Printed here, bailed on at the end: the bail order is load-bearing for the
        // self-test, and the diagnosis stays visible either way.
        ...
        // After the violations, before everything else: a corrupt read outranks this, but a
        // wedge *causes* every later check (no join, frozen rings, floors tripping), so
        // reporting one would bury the cause.
        ...
        // §3.5 before the read floor: an unrecovered migration *causes* the floor to trip.
        ...
        // A run that killed nobody is not §11.4's workload.
        ...
        // A run that validated nothing must not print PASS: "the observer never managed a
        // lookup" is a thing that did not happen, and printed the same verdict as one that
        // did.
        ...
        // The self-test's other half, before the floor as the more specific diagnosis:
        // `--inject-violation` publishes a NaN all run and must fail.
        if a.inject {
            bail!(
        ...
        // A run nobody was writing to proves nothing and does not trip the floor above:
        // four frozen rings that overlap answer every composed read forever (see
        // [`RoundHealth`]).
        ...
        // `--crash-points` that exercised nothing is a failure: a workflow reads only the
        // exit status, so `armed 0, aborted 0` must not exit 0
        // (`a_run_that_validates_nothing_fails_instead_of_passing` is the shape one level
        // down).
        if a.crash_points {
            if ledger.armed == 0 {
                bail!(
    ...
    struct Migration {
        ...
        rounds: u64,
        read_before: bool,
        /// The kill was **not performed**: the census showed no read-write survivor besides
        /// the role holder.
        deferred: bool,
        /// Read-write participants attached **before** the kill, the role holder included,
        /// or `None` when the marker named nobody.
        heirs_before: Option<u64>,
        /// Live participants other than the driver's observer, censused the instant the
        /// victim was reaped: the heirs that existed when the role fell vacant.
        heirs_at_kill: Option<u64>,
        /// The same census on the first round after the kill; the pool drains during a
        /// vacancy, so only the pair says which population a late recovery had.
        heirs_first_round: Option<u64>,
        /// Wall time from `kill()` to `wait()` returning: **the blind window** in which the
        /// pool can drain unseen (a survivor's detach arm runs while the marker still names
        /// the corpse).
        kill_to_reaped: Option<Duration>,
        /// Diagnostic only: the slots behind `heirs_before`, from the same census pass.
    ...
    impl Migration {
        ...
        fn line(&self) -> String {
            ...
            if self.deferred {
                // The census is the whole content: it separates a thin fleet from
                // the absorbing state.
                return format!(
            ...
            // `heirs_before` and the reap window print because `heirs_at_kill =
            // heirs_before - 1 - departures` is otherwise not recoverable from a
            // nightly's log.
            format!(
        ...
        fn failure(&self) -> Option<String> {
            ...
            // §3.5, NORMATIVE: "Lookups do not stop, slow down, or observe anything
            // during a takeover.
    ...
    /// `SIGKILL` whichever process currently holds the rendezvous, then require what §3.5 owes.
    ...
    fn kill_the_owner(
        ...
        read_before: bool,
        stop_owner_ms: u64,
        // `--defer-owner-kills`: defer this attempt, applied *after* the census so
        // `heirs_before` and `starved` stay truthful.
        force_defer: bool,
    ) -> Migration {
        let mut m = Migration {
            n,
            ...
            heirs_before: None,
            kill_to_reaped: None,
            // Reserved before any window opens, so the census inside it does not
            // allocate.
        ...
        // §3.5's precondition is established, not raced for: §11.3's `takeover.*` row
        // presumes another participant (see [`Migration::deferred`]), and this is the only
        // instant at which whether one exists is decidable.
        ...
        // Brackets both arms, including the deliberate hold below, which is part of the
        // blind window the printed figure is compared on.
        ...
        // [diag] Instrument 5 at a driver kill: an owner that aborted and is still dumping
        // holds its byte and counts in the census.
        ...
        // Censused here and nowhere else: `drive`'s round census is stale across the
        // interval in which the pool changes.
        ...
        // Read and probe in one loop: the observer keeps validating while the role is
        // vacant (§3.5's data-plane claim), and the fresh join says the role stopped being
        // vacant.
        ...
        while start.elapsed() < OWNER_RECOVERY_DEADLINE {
            ...
            // `ReadOnly` because a consumer is what a robot restarts.
            let joined = tf_tree::Open::new()
                .mode(AttachMode::ReadOnly)
                .create(CreatePolicy::Never)
                // Short: a long timeout would measure the timeout, not the
                // recovery.
        ...
        // The evidence trails the recovery: `Tree::inherit_ownership` renames the socket in
        // *before* the heir reaches its `publish_owner_pid` and `record_inheritance`
        // writes.
        ...
        // [diag] Instrument 5's line for this kill, written now that nothing is timed.
    ...
    /// Collect children that have exited, recording the ones that reported a violation.
    fn reap_finished(
        kids: &mut [Option<Kid>],
        violations: &mut Vec<String>,
        ledger: &mut CrashLedger,
        ...
        for (i, slot) in kids.iter_mut().enumerate() {
            ...
            match kid.proc.try_wait() {
                Ok(Some(status)) => {
                    // [diag] Instrument 5's two clock reads, taken at the reap.
                    let reaped = Reaped::now(status);
                    // A child that aborted at an armed §11.3 site is counted so
                    // the run can say sites *fired*.
                    #[cfg(unix)]
                    {
                        use std::os::unix::process::ExitStatusExt as _;
                        if status.signal() == Some(libc::SIGABRT) {
                    ...
                    // a worker in `kids`, so an heir aborting in its own hangup
                    // callback is reaped here.
    ...
    /// Join the arena the owner child created, and hold a reader on it for the whole run.
    ...
    /// Probe every chain edge for the window it currently retains, by a deliberate
    /// `Extrapolation` an hour past the shared clock (`Plan::at` reports `oldest`/`newest`
    /// exactly when it refuses, and the probe never succeeds or disturbs a ring).
    ...
    struct RoundHealth {
        /// How many chain edges reported a window at all.
        windows: usize,
        /// Did the four windows intersect?
        overlap: bool,
        /// Width of the intersection, ns (`overlap`), or how far apart the nearest pair was
        /// (`!overlap`).
        width: i64,
        gap: i64,
        /// Index into [`CHAIN`] of the edge whose `newest` was the minimum — the one
        /// holding the intersection back — and of the edge whose `oldest` was the maximum.
        laggard: usize,
        blocker: usize,
        /// Composed reads that succeeded, of the 256 attempted.
        chain_ok: u64,
        /// `now - newest` for the *freshest* chain edge, ns: whether anybody is still
        /// writing.
        freshest_age: Option<i64>,
        /// Chain edges whose claim word names a participant that is still alive / is
        /// already dead / is free.
        writers_live: u64,
        writers_dead: u64,
        /// Participant slots holding a `LIVE` record, and slots the kernel agrees are
        /// alive.
    ...
    struct Health {
        ...
        /// Denominators for the averages below: a round with no window contributes no age,
        /// and one with an intersection contributes no gap.
        gap_rounds: u64,
        freshest_rounds: u64,
        freshest_sum: i64,
        freshest_max: i64,
    ...
    impl Health {
        ...
        /// Milliseconds, or `n/a` when the quantity was never observed (a mean over zero
        /// samples is nothing): on `freshest` no edge was ever written, on `gap` the
        /// windows always intersected.
        ...
        fn diagnosis(&self) -> String {
            let r = self.rounds.max(1) as f64;
            // `n/a` when no round was non-overlapping: `laggard` is then all zeroes and
            // `max_by_key` would still name an edge.
    ...
    /// [`census`], optionally keeping each slot it counted.
    fn census_with(tree: &Tree, h: &mut RoundHealth, mut seen: Option<&mut Vec<SlotSeen>>) {
        let view = tree.arena_view();
        // `1..=CHAIN.len()`: `EdgeId` is 1-based (`TreeBuilder::build_with` sizes the table
        // `declared + 1`, leaving id 0 an unclaimable sentinel); a `0..` walk never looked
        // at `arm->tool`.
        ...
        for edge in 1..last {
            ...
            // `slot_of`, not a hand-rolled `& 0xFFFF`: the owner word packs `(epoch <<
            // 16) | (slot + 1)` and reserves a `CLAIMING` sentinel only the helper
            // knows.
    ...
    /// A burst of checked reads from the never-killed observer, and a [`RoundHealth`]
    /// describing the arena they came out of.
    fn observe(
        tree: &Tree,
        rng: &mut Rng,
        violations: &mut Vec<String>,
        ...
        census(tree, health);
        // `let Some(..)`, not `expect`: `None` only when no edge reported a window, and a
        // soak must not panic over a diagnostic.
    ...
    /// Slots holding a `LIVE` record for a process the kernel says is dead, from the caller's
    /// own attachment and excluding its own slot.
    ...
    /// §11.4's "participant and claim slots never leak", checked once the run is quiescent, on
    /// the observer's attachment held for the whole run (the arena the children tortured, not a
    /// fresh one).
    fn check_recovery(
        tree: &Tree,
        ...
        // `a_run_that_never_migrates_holds_every_worker_record_to_the_strict_path` green
        // while asserting nothing (that test reads this line).
        out.notes.push(
            if hangup_collector_pinned {
        ...
        // Count what the dead left before reclaiming it: `reap_dead`'s return cannot tell
        // "nothing to reclaim" from "this step did not run".
        ...
        // Reap after the count and before the claim probe: a claim held by a process the
        // kernel has cleaned up is reapable, not leaked (A3).
        ...
        for (parent, child) in CHAIN {
            ...
            // Claiming is the sharpest test available: it succeeds only if no owner
            // remains.
        ...
        // [`census`]).
        /// How long the kernel gets to finish tearing the killed children down before the
        /// strict verdict is read: eight 250 ms steps.
        ...
        // The records no hangup callback can reach are separated by pid, and first.
        ...
        leaked = others;
        // On a run whose role migrated the partition is reported, not enforced: the strict
        // half presumes an owner was alive and serving while these sockets closed.
    ...
    fn child(argv: &[String]) -> Result<()> {
        ...
        // [`Args::victim_ballast_mb`]): a window on a plain release build in one flag.
        ...
        // process, `SLOW_JOIN_RUN_LINES` per run, and one refusal and one join line per
        // episode.
        ...
        loop {
            ...
            // that created a second would split the run into a green one that validates
            // nothing.
            ...
            drop(tree);
            // [diag] Instrument 3's two writes, after the drop so neither lengthens the
            // tenure `CapExit` reports; the ledger line only when this attachment
            // inherited.
    ...
    /// The random-operation loop against one attachment.
    fn work(
        tree: &Tree,
        dir: &Path,
        rng: &mut Rng,
        ...
        // The edge index travels with the writer: §11.4's two-writers check reads its claim
        // record from inside the holder (`EdgeWriter` exposes only `push`, and an accessor
        ...
        // Whether this attachment is parked at its cap awaiting a kill window, so the stall
        // is recorded once, not once per re-check.
        let mut capped_in_window = false;
        // [diag] Instrument 3's counters, each advanced only on a rare branch:
        // one-operation budgets granted while a kill window holds the cap, and when this
        // attachment took the role.
        ...
        while ops_left > 0 {
            ...
            // way, so it asks the same two questions: am I the role holder.
            if ops_left == 0
            ...
            // Pacing, not politeness: an unthrottled loop covers ~9 microseconds of a
            // 64-slot ring and busy-waits every core, so the kills land in one hot loop
            // instead of across the protocol.
            std::thread::sleep(Duration::from_micros(200 + rng.below(1_600)));

            // §3.5's trigger, and the participants are the callers: there is no daemon
            // (`docs/decisions/0019`), so a survivor that never evaluates `owner_lost`
            // never becomes owner.
            if !no_inherit && tree.owner_lost() {
                ...
                // whether the trigger was ever *answered*.
                ...
                if let Ok(Inheritance::Inherited) = outcome {
                    // pid first, then the log line: the driver reads the pid to
                    // pick the next victim, so it must never be behind.
            ...
            match rng.below(100) {
                ...
                0..=9 => {
                    if held.is_none() && !readers_only {
                        ...
                        let (p, c) = ids[i];
                        // A refused claim is correct when somebody else
                        // holds it.
                ...
                // **Sweep the participant table**, which is a different collector
                // from `reap_dead` above: that one reclaims *claims* held by dead
                // writers, this one reclaims the dead participants' *records*
                // (`docs/decisions/0028` plan step 5).
                17 => {
                    let _ = tree.reap_participants();
                }
                // Detach and re-join — **unless this process is the one serving the
                // rendezvous.**
                18..=19 => {
                    if read_owner_pid(dir) != Some(std::process::id()) {
                        // **Checked last, immediately before leaving.** The
                        // driver opens the window before it signals, so a
                        // child that reads `false` here is reading it at an
                        // instant when the owner is still alive.
                ...
                20..=59 => {
                    if let Some((edge, w)) = &held {
                        ...
                        // checked on every push instead of once at
                        // teardown.**
                        let word = tree
                            .arena_view()
                            .claim(w.edge())
                        ...
                        // `ClaimRevoked` is A4 working: this writer was
                        // judged dead, reaped, and is being fenced.
                ...
                _ => {
                    ...
                    // stamp that all four rings can answer is an intersection
                    // of what they retain.
                    let windows = edge_windows(tree, &guard);
                    let Some(window) = common_window(&windows) else {
                        continue;
                    ...
                    if let Ok(iso) = plan.at::<tf_tree::SystemDomain>(&guard, Stamp::from_nanos(at))
                    {
                        if let Err(why) = Invariant::check(&iso) {
                            ...
                            // Exit immediately: the arena has already
                            // told this process something impossible,
                            // and every further read would report the
    ...
    /// `[diag]` instrument 3's reading at a cap exit, taken in [`work`] while the attachment is
    /// still held and written by [`child`] after the drop.
    struct CapExit {
        /// Whether the marker named this process at the cap exit.
        named: bool,
        attachment_ops: u64,
        extensions: u32,
        /// Since this attachment inherited.
    ...
    impl CapExit {
        /// **`heirs_attached_after_exit` means what the driver's post-kill count means**
        /// (`N after it` on a `§3.5 owner kill` line): read-write participants alive other
        /// than the departing role holder and the driver's observer, which never inherits.
        fn line(&self) -> String {
    ...
    /// A random rigid transform — or, under `--inject-violation`, one that is not rigid at all.
    ...
    mod tests {
        ...
        /// `/proc/self/limits` as the kernel prints it, with the core row set to
        /// `soft`/`hard` and one neighbour on each side, so the parse has to find its row
        /// rather than read the first one.
        ...
        /// configuration**: armed crash points, a `|` `core_pattern`, and a soft
        /// `RLIMIT_CORE` that is not 1.
        #[test]
        fn the_core_dump_warning_names_only_a_dumping_pipe() {
            ...
            // The kernel reads the SOFT limit. soft=1 with a looser hard limit (what
            // the ASan build read without the prefix) is suppressed...
            assert_eq!(
