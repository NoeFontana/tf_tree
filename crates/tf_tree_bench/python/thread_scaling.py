...
Measured on the development host, `python3.14t` 3.14.2 with the GIL disabled, 4
physical cores / 8 logical CPUs, batch 64, at this file's default window — seven
runs, and three more of the `--serialize` control.

**The finding is that scaling is linear to the physical core count.** The 4-thread
arm sits a few percent under 4x on four cores in every run; the 2-thread arm sits
just under 2x. That is the claim §12.2 criterion 4 exists to make and it is now a
number rather than a hope. The 8-thread arm adds roughly a quarter on top of the
4-thread one, which is what SMT adds — so **6x is out of reach here because of the
core count, not because of the code**, and the binary says `INVALID` rather than
`FAIL`.

**No interval is quoted, per `docs/benchmarks/EVIDENCE.md`'s rule**: a range over
seven runs is a sample and not this instrument's spread, and re-running walks
outside one without anything having changed. `just py-thread-scaling` prints the
run's own numbers.

**The control separates the two decisively.** `--serialize` reads *below* 1.0x at
every thread count in all three runs — it gets slower with more threads, as lock
contention does — against an almost-4x 4-thread arm. There is no reading of the
noise under which those two are the same measurement.

**The window is load-bearing and short runs understate scaling.** At
`--seconds 1` the 4-thread arm read 3.29x in one sitting against 3.9x-plus at the
default 2 s. The defaults are the measured configuration; shortening them to make
the sweep quick reports a worse machine than you have.
"""
...
def main() -> int:
    ...
    if gil:
        ...
        print(
            f"  PASS — {scaling:.3f}x over {widest} threads, criterion 4 wants >= 6x."
        )
        verdict = "PASS"
    else:
        print(
            f"  FAIL — {scaling:.3f}x over {widest} threads, criterion 4 wants >= 6x."
        )
        verdict = "FAIL"
